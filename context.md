...
**[ВЫСОКОЕ] Продуктовый слой отключён от стека.** `aggregator/batcher.py`, `aggregator/api.py`,
`sdk/python/qlsa`, `sdk/js` выдают VFRI7–VFRI10. Дефолтный стек — v7 (VFRI11), рекурсивный — v8.
Хинты VFRI10 верификатором VFRI11 не принимаются **by design** (другой хеш-бэкенд → другой trace
root → другие FRI query indices). До деплоя доходит только `testnet/e2e.py`. Это не техдолг,
а разрыв продукта: сервис как таковой сквозного пути не имеет.
