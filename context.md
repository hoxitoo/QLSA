...
## 2026-09-11 — аудит Ф1: четыре бага, один серьёзный; legacy возвращён в архив

Проверка по просьбе владельца после сужения. Нашлось четыре поломки, и одна
из них — процессный промах, а не просто баг.

### Б1. Rust-тесты не компилировались — и почему я этого не увидел

```
error[E0433]: use of undeclared type `P2Channel`     vfri2_bridge.rs:3683
error[E0425]: cannot find function `hash_pair_p2`    vfri2_bridge.rs:3706
```

Я удалил t=2-хелперы, потому что **`cargo build` сообщил «never used»**. Но
`cargo build` не компилирует `#[cfg(test)]`, поэтому его «не используется»
слепо к каждому вызову из тестов. `cargo test` не собирался вовсе.

Хуже самого бага — последовательность: я прогнал полный набор (526 тестов),
**потом** удалил хелперы, и записал в коммит прежний замер. Ровно тот механизм,
который проект документирует как повторяющийся: измерить, изменить, отчитаться
старым измерением. Правило записано в `CLAUDE.md`: **для решения «мёртвый ли
код» использовать `cargo test`, не `cargo build`.**

### Б1-бис. Удаление по ИМЕНИ модуля унесло живой код

Разбираясь с Б1, обнаружил худшее. Генераторы фикстур
`write_v23_vfri12_fixture`, `write_vfri12_e2e_fixture`, `write_vfri11_e2e_fixture`
и `write_outer_width_probe` жили **внутри `mod tests_vfri8`** — модуля,
названного по ретированному протоколу, но содержащего несущий код боевых
VFRI11/VFRI12. Я снёс модуль по имени, и вместе с ним — возможность
перегенерировать фикстуры.

Починено правильно: модуль возвращён целиком, а затем компилятор назвал 15
идентификаторов, которых действительно нет, и по ним удалены 29 тестов. Это та
же дисциплина, что в самом сужении («компилятор — судья, не grep»), но в первый
раз я применил её к файлам и не применил к модулям.

### Б2. `benchmarks/bench_witnesses.py` падал

`ImportError` на `prove_mldsa_sig_vfri7_stark`. Бенчмарки не покрыты тестами —
поэтому и не всплыло. Переведён на `prove_mldsa_sig_vfri11_stark`; добавлен
`tests/test_benchmarks_import.py`, проверяющий импортируемость каждого
бенчмарка, чтобы следующее удаление сломало их громко.

### Б3. `.github/workflows/deploy.yml` был мёртв

Запускал удалённый `scripts/deploy.js` и грепал вывод на `QLSAVerifierBound` /
`BatchRegistryV2`. Заметка: workflow устарел **до** сужения — он деплоил стек
MVP-1. Переведён на `deploy_v7.js` + `QLSAVerifierVFRI11` / `BatchRegistryV5`,
причём грепы сверены с тем, что скрипт реально печатает.

### Б4. Два `unused import` в тестах — та же первопричина, что Б1

### Что НЕ сломалось

Python (503 → 508 с новыми), `mypy --strict`, Solidity 250, e2e dry-run — всё
проходило и до починки. Поля `vfri7..10` в `sdk/js/src/client.ts` и
`BatchResult` намеренно оставлены read-only-представлениями ради стабильности
HTTP-ABI; это задокументировано, а не забыто.

## legacy возвращён: `archive/` вместо удаления

В утверждённом `ROADMAP.md` § Ф1 написано «остальное — в `docs/archive/` или
отдельную ветку». В плане Ф1 я свёл это к удалению с тегом и так и сделал —
отклонение от собственного документа, которое владелец заметил.

Восстановлено **112 целых файлов** плюс фрагменты, которых целыми файлами
никогда не было: 36 Rust-мостов, 23 PyO3-биндинга, t=2-хелперы с их тестами,
28 Python-обёрток, 115 Python-тестов — извлечены из `f2020d9` в справочные
файлы с шапкой, называющей источник.

**Архив инертен — проверено, а не заявлено:** после его добавления hardhat
компилирует те же **36** контрактов, а `pytest --collect-only` из корня
собирает **0** файлов из `archive/`.

При извлечении дважды сработал подстрочный матч: `fn hash_leaf_cols_p2` —
подстрока живого `..._p2t8`, поэтому фильтр «уже удалено?» ошибался. Исправлено
на границы слов; тот же класс ошибки, что и в самом сужении.

### Состояние

**250 Solidity + 518 Rust + 508 Python**, 0 падений, 0 предупреждений,
`mypy --strict` чист.


## Ф1 (2026-09-11) — поверхность сужена с 28 верификаторов до 3

Первый шаг `ROADMAP.md` после его создания. Причина не косметическая: внешний
аудит оплачивается по объёму, VFRI5–VFRI8 лежали в `src/` с пометкой «не
деплоить», и каждая лишняя версия была местом, где документация расходится с
кодом, — что в этой работе случалось трижды.

| | было | стало |
|---|---|---|
| `QLSAVerifier*.sol` | 28 | **3** (VFRI11, VFRI12, Recursive) |
| `BatchRegistry*.sol` | 7 | **2** (V5 прямой, V7 рекурсивный) |
| файлов в `verifier/` | 22 | **13** |
| строк Solidity | 13 835 | **4 404** |
| тестовых файлов Solidity | 65 | **21** |
| `vfri2_bridge.rs` | 10 872 | **6 052** |
| `stark/prover.py` | 3 656 | **2 662** |
| контрактов «не деплоить» в `src/` | 4 | **0** |

Точка возврата — коммит `f2020d9`, родитель этого изменения. История git и есть
архив; тег `pre-narrowing` поставлен локально, но пуш тегов в этот репозиторий
закрыт политикой, поэтому ссылаться надо на SHA — он в истории ветки и доступен
любому, кто её клонирует.

### Выбор между t=8 и t=16 намеренно отложен

Оба бэкенда оставлены. VFRI11 (t=8) даёт запас по газу 10,7M, VFRI12 (t=16) —
128-битные узлы при 8% запаса. Что из этого боевое, станет ясно, когда Ф2
измерит стоимость доказательства членства и корня дерева. Решать это до замера
значило бы гадать.

### Процедура: компилятор — судья, а не grep

Ключевая деталь исполнения. `grep` совпадает с комментариями: внутри
`QLSAVerifierVFRI11` нашлись `Poseidon2ChannelT4` и `Poseidon2MerkleVerifierT4`,
но это **NatSpec о миграции** (строки 21–22), а не импорты. Решение принималось
по `import`-строкам, а окончательную проверку делал `solc`.

Это же дважды спасло от лишнего удаления. Автоматический проход по телу функции
снёс `abi_encode_vfri9_hints` и `gen_vfri_hints_from_cols_nfolds` — обе нужны
VFRI11 и VFRI12, потому что **queryHints ABI байт-в-байт одинаков у VFRI9…VFRI12**
и различается только хеш-бэкенд. Компилятор это поймал, обе восстановлены;
энкодер при этом переименован в `abi_encode_query_hints` — старое имя указывало
на контракт, которого больше нет, и именно поэтому под автоматическое удаление
и попало.

### Тест, ставший содержательнее

`QLSAVerifierVFRI11E2E` проверял, что hints t=8 отвергаются верификатором t=4.
VFRI10 снят, поэтому проверка переведена на **VFRI12** — и стала лучше: t=8 и
t=16 это два бэкенда, которые реально отгружаются, так что спутать можно именно
их. ABI у них одинаков, значит различает их только хеш — что и должно быть
достаточным.

### Слияние сабмиттеров

`OnchainSubmitterV5` наследовал от `OnchainSubmitterV4`, добавляя две константы:
у BatchRegistryV4 и V5 байт-идентичные ABI. V4 снят вместе с остальным legacy,
поэтому классы слиты в один — под именем реестра, который существует.

### О числе тестов

**250 Solidity + 526 Rust + 503 Python** (было 1092 / 596 / 619). Падение — это
исчезнувшее покрытие исчезнувшего кода, а не потеря покрытия: каждый общий
компонент (`M31`, `QM31`, `CirclePoint`, `Blake2s`, `RecursiveChannelReplay`,
оба бэкенда Poseidon2) сохранил выделенный тестовый файл, а боевой путь от
подписи ML-DSA-65 до `submitBatchWithNonces` проходит целиком. Сказано прямо,
потому что цифра иначе читается как регрессия.


## 2026-08-28 — коллизия padding'а в губке Poseidon2 (t=8, t=16)

Найдено при работе над A-5. Строя батч-дерево на Poseidon2-t8, я читал
`sponge_t8`, чтобы понять, что придётся арифметизировать, и увидел в хвостовой
ветке прибавление **константы 1** в ячейку ёмкости. Тот же класс дефекта, что я
чинил в канале t=16 (R4.22), но в другой функции — губке, а не канале.

**Коллизия подтверждена зондом:**

```
t4  (rate 2): [1] vs [1,0]           -> differ    ← иммунен
t8  (rate 4): [1] vs [1,0]           -> COLLIDE
t8  (rate 4): [1,2] vs [1,2,0]       -> COLLIDE
t16 (rate 8): [1] vs [1,0]           -> COLLIDE
t16 (rate 8): [1,2,3] vs [1,2,3,0,0] -> COLLIDE
```

Дополненный блок помечался константой, поэтому внутри одного частичного блока
**хвостовые нули невидимы**: два входа, чьи последние неполные блоки различаются
только нулями в конце, хешировались одинаково.

| ширина | rate | длины остатка | статус |
|---|---|---|---|
| t=2 | 1 | частичных блоков нет | иммунен |
| t=4 | 2 | только одна | иммунен |
| **t=8** | 4 | 1, 2, 3 | **был уязвим** |
| **t=16** | 8 | 1…7 | **был уязвим** |

### Почему существующие тесты не поймали

`test_sponge_full_block_vs_padded` (t=16) и
`test_sponge_padding_distinguishes_lengths` (t=4) проверяют границу «полный блок
против дополненного» — то есть где флаг появляется или исчезает. Ни один не
проверял **два остатка разной длины внутри одного блока**, а это и есть
коллизия. Замороженные векторы `SPONGE_1_TO_12` (добавлены в R4.9 именно чтобы
закрепить хвостовую ветку) сделали Rust и Solidity доказуемо одинаковыми —
добросовестно, вместе с дефектом. Закрепление двух реализаций друг к другу не
проверяет ни одну из них.

### Эксплуатируемость

**Сегодня — нет.** Каждый вызов `hashLeaf` on-chain передаёт ровно 4 слова
(`_qm31ToWords`), длина не варьируется. Но дефект латентный в примитиве, а
`batch_tree.rs` — код, который я писал в тот же день, — стал первым потребителем
с **переменной** длиной. Там его спасало только слово длины в кодировании; это
хрупко, следующий вызывающий его бы не получил.

### Правка

Дополнение теперь несёт **длину блока**, а не факт своего наличия:
`state[last] += rate − rem.len()`. Для t=4 это то же значение (rate 2 даёт 1),
но правило записано единообразно для всех ширин — исключение в одной из трёх и
есть то, из чего вырастают расхождения.

Исправлено в Rust (`poseidon2_t4/t8/t16.rs`) и в Solidity
(`Poseidon2M31T4/T8/T16.sol`). NatSpec t=8 утверждал верное, но не то свойство:
«никакой выбор данных не подделает дополненный блок» — истина, не покрывающая
коллизию двух дополненных блоков между собой.

### Асимметрия как проверка корректности

Ключевой признак того, что правка задела только дополненные блоки:

- **t=8 не изменился нигде, где это важно.** `hashLeaf` получает 4 слова = ровно
  один полный блок при rate 4, дополнения нет. Боевой стек VFRI11 не затронут:
  все фикстуры t=8 валидны без изменений, `v23_dual_submitBatch_t8` — дрейф
  **0,00%**, `outer_recursion_verify_t8` — **0,00%**.
- **t=16 изменился.** 4 слова при rate 8 — частичный блок. Перегенерированы
  `vfri12_e2e.json`, `full_v23_vfri12_cross_bound_e2e.json`,
  `outer_width_probe.json` и вектор `REF_T16_LEAF`.
- Из замороженных векторов t=8 сдвинулись только n ≡ 1, 2 (mod 4): остаток 3 и
  так прибавлял 1, а остаток 0 дополнения не имеет. Ровно поэтому векторы и не
  ловили коллизию.

Газ: `v23_dual_submitBatch_t16` 15 436 509 → **15 432 163** (0,03% — байты
calldata, не число операций); `outer_recursion_verify_t16` без изменений,
отношение ширин 2,95× сохранилось.

### Добавлены тесты, которых не хватало

В каждой из трёх ширин — тест на два остатка **разной длины внутри одного
блока**. Написан и для t=4, который иммунен: держать надо правило, а не
случайность, которая одну ширину спасла.

1092 Solidity-теста, 598 Rust, 619 Python.


## Аудит 2026-08-27 — три прохода по дереву агрегации

Первый аудит нового кода: гаджет канала t=8, узел агрегации, сборка дерева,
PyO3-биндинг и Python-обёртка.

### Крипто

**Чисто:** секретов в дереве и истории нет (`.env`/`.env.deployed` в `.gitignore`,
трекается только `.env.example`; совпадения на 64-hex — это RFC-векторы Blake2s).
Все три верификатора `composition_channel_t8` проверяют `proof.commitments.len()`
до индексирования — вредоносное доказательство даёт ошибку, а не панику.
`node_shape` валидирует КАЖДОЕ утверждение: непустые запросы, соответствие
`paths`/`comp_paths` числу запросов, длину layout'а, число свёрток и совпадение
challenges каждого запроса с транскриптом. Каппы `MAX_QUERIES`/`MAX_NUM_FOLDS`/
`MAX_DEPTH` на месте.

**Найдено — A-5, членство подписей в батче не доказывается.** `batch_merkle_root`
входит в лист только как связывание (Fiat-Shamir + commitment); путей включения
в схеме нет. Корень утверждает «N подписей проверены ПОД корнем R», но не «эти N —
состав R». Не эксплуатируемо сегодня (дерево не в цепи), но дерево делает разрыв
**тоньше**: раньше было очевидно, что доказывается одна подпись из N, теперь
недоказанным остаётся ровно одно звено, которое легко счесть закрытым.
Закрывается тем же `merkle_path_t8_air` — один путь на лист. Подробности в
`docs/TECH_DEBT.md` § A-5.

**Удалено:** `contracts/scripts/_scratch_one.js` — черновой скрипт деплоя
`QLSAVerifierVFRI11` без аргументов, попавший в репозиторий. Вреда не наносил, но
запускался против любой сети из конфигурации.

### Код

**Мёртвая работа в сборке дерева (исправлено).** `prove_aggregation_tree`
вычисляла утверждение для КОРНЯ — полное извлечение `gen_vfri11_recursion_inputs` —
и отбрасывала его, потому что `break` срабатывал после. Там же строились колонки
трейса корня, которые никто не потребляет: над корнем нет уровня. Оба вычисления
теперь пропускаются на корневом уровне; `TreeLevel.columns` документирован как
пустой на корне.

*Честно об эффекте:* работа провабельно мёртвая, но выигрыш КОНСТАНТНЫЙ (одно
извлечение и одно построение трейса на всё дерево), а разброс замера — 23%
(прогоны 10,2 / 8,3 / 8,3 с на четырёх подписях). Ускорения это не даёт и не
должно было; это удаление мёртвого кода, а не оптимизация.

**Расхождение документации с кодом (исправлено в документе).** A-4 утверждал
«уровни параллельны. При 8 воркерах N=128 занимает ≈2,7 минуты». Уровни
параллельны ПО СТРУКТУРЕ, но `prove_aggregation_tree` доказывает узлы обычным
последовательным циклом — рабочих потоков в этом пути нет. Оценка описывала
возможность как поведение. Фактически N=128 это ≈15 минут на одном ядре.
Это тот самый механизм дрейфа, который разбирался в аудите 2026-07-31, — поэтому
поправка стоит в тексте, а не заменяет его молча.

**Проверено чисто:** продуктовый слой (rate-limiter вытесняет устаревшие окна,
доверенные прокси валидируются, API не отдаёт `str(exc)`), широкий `except` в
`HttpClient.health()` — верное поведение для health-check'а.

### Оптимизации (предложены, не внесены)

- **C-7 параллельные узлы внутри уровня** — узлы уровня независимы. Линейное
  ускорение по ядрам: N=128 с ≈15 минут до ≈2,7 при 8 воркерах. Это и есть та
  оценка, что стояла в A-4 как факт.
- **C-8 трейс узла строится дважды** — `prove_tree_node` и
  `tree_node_trace_columns` строят одни и те же три трейса. Единицы процентов на
  фоне 6,82 с доказательства, поэтому ПОСЛЕ C-7.
- **C-9 вытеснение в rate-limiter'е под глобальной блокировкой** — O(n)-скан всех
  трёх словарей раз в 500 запросов, удерживая lock; всплеск задержки при большом
  числе IP. Амортизация даёт O(1) на запрос.

579 Rust-тестов, 0 падений.


## 2026-07-31 — техдолг сведён в один документ

`docs/TECH_DEBT.md` — единый список открытого, разделённый по тому, ЧТО мешает
пункту сдвинуться, а не по теме:

- **Блок A (решения владельца):** доверительная модель (хеш-шаг вне схемы),
  что означает «агрегация» (доказывается одна подпись из N), изменение
  интерфейса `MAX_SENDERS`. До решения писать код бессмысленно — неясно, какой.
- **Блок B (ресурсы и доступы):** прогон на публичном тестнете, внешний аудит.
- **Блок C (можно делать сегодня):** rate-8 squeeze в t=16 (~520k газа на
  группу), батчевая инверсия M31, кеш witness'ов, `_matI` через сдвиги,
  упаковка storage, мутабельный `inner` во frozen-датаклассе.

Туда же вынесен разбор путей по доверительной модели: Keccak-AIR (месяцы,
trustless), явное доверие с бондом/порогом/TEE (продукт сегодня), гибрид.
Рекомендация — гибрид с приоритетом «N подписей» перед «trustless»: сегодняшнее
«одна из N» это не половина продукта, а его отсутствие, потому что экономии
calldata нет, а приходят именно за ней; и решается оно расширением уже
построенной рекурсии, а не новой стройкой.

Инструкция по публичному тестнету — в README, раздел «Public testnet run».
Попутно исправлено: `hardhat.config.js` жёстко зашивал публичный RPC для sepolia
и игнорировал `RPC_URL`, то есть первый шаг инструкции настраивал эндпоинт,
который второй шаг не использовал.


## Аудит 2026-07-31 (второй проход) — продуктовый слой

Три прохода по коду, добавленному после первого аудита. Все находки — в коде, написанном
в этой же сессии; это важно как показатель: новый слой требует такой же проверки, как старый.

### A1 — криптография и безопасность

**[ВЫСОКОЕ, исправлено] Рекурсивный маршрут молча работал на 16 битах.**
`prove_mldsa_sig_for_protocol` имел `n_queries: int = 1` и всегда его передавал, поэтому
собственный дефолт `prove_mldsa_sig_recursive_stark` (20) не мог примениться никогда. Вызывающий,
не знавший, что нужно просить 20, платил за рекурсию и получал 16 бит
(`log_blowup(6) × n_queries + pow_bits(10)`) — при этом ДОРОЖЕ прямой проверки. Это единственная
конфигурация, в которую рекурсию нельзя входить случайно, и `testnet/e2e.py --stack v8` специально
её предотвращает. Мой же docstring утверждал, что «значение вызывающего побеждает при явном
указании» — верно и бесполезно: вызывающий указывал всегда.

Исправлено: `n_queries=None` означает «дефолт этого протокола», таблица одна
(`PROTOCOL_DEFAULT_QUERIES`), `Batcher.n_fri_queries` по умолчанию `None`. Проверено: батч с
`witness_protocols=("recursive",)` без указания числа запросов даёт `nQueries=20`, 130 бит.

**[СРЕДНЕЕ, исправлено] Признак формы реестра мог разойтись с полезной нагрузкой.**
API помечал доказательство `direct`/`recursive` поиском имени в рукописном `DIRECT_PROTOCOLS`.
Добавить рекурсивный протокол и забыть про список — и доказательство уедет в реестр не той формы,
падая внутри транзакции, а не на границе. Это тот же класс отказа, что и сам разрыв продуктового
слоя. Теперь признак выводится из НАГРУЗКИ (`WitnessGroup.inner`), а `DIRECT_PROTOCOLS` выводится
из реестра, а не пишется вторым списком.

**Проверено и чисто:** секретов в индексе нет, bandit чист, release-сборка Rust без предупреждений,
на продовом пути генерации хинтов нет `unwrap`, внешние входы ограничены (`n_queries` 1..64,
`tree_depth` 2..=30, ёмкость мемпула, история батчей).

### A2 — код, баги, тупики

**[СРЕДНЕЕ, исправлено] SDK глотал неизвестное имя протокола.** `_prove_witness_local` ловил
`KeyError` внутри цикла и продолжал, поэтому запрос несуществующего протокола возвращал
`has_witness=False` без объяснения — та самая «тихая пропажа», ради устранения которой реестр и
писался, и расхождение с `Batcher`, который на том же входе падает. Теперь имена проверяются
заранее, а оставшийся `except` сужен до рантайм-условий.

**Замечено, не менялось:** `WitnessGroup` объявлен `frozen=True`, но `inner` — мутабельный словарь,
так что заморозка на него не распространяется. Один производитель, не разделяется, а иммутабельная
обёртка усложнила бы сериализацию без реальной угрозы — зафиксировано, а не переделано.

**`HttpClient.health()`** глотает любое исключение — для health-check это корректное поведение,
а не дефект.

### A3 — предложения по оптимизации (не реализованы)

| Предложение | Эффект |
|---|---|
| Строго возрастающий `senders` вместо O(n²) дедупликации | предел ~212 → ~480 на V5, ~70 → ~150 на V7; цена — изменение интерфейса |
| Rate-8 squeeze в `Poseidon2ChannelT16` (absorb уже rate-8, squeeze остался rate-2) | ~19 перестановок на verify → ~5; экономия ~520k газа на группу |
| Батчевая инверсия M31 (трюк Монтгомери) в `vfri_fri_chain` | 1024 инверсии при LOG=10 → 1 инверсия + ~3072 умножения; только скорость prover'а |
| `_matI` в `Poseidon2M31T16`: `mulmod` для μ ∈ 1..16 заменить сдвигами | оценочно 5–8% перестановки t=16; **не измерено** |
| Кеш доказательств по `(protocol, merkle_root, tx0)` в `Batcher` | повторная генерация того же witness'а при ретрае батча сейчас считается заново |


## Аудит 2026-07-31 — что доказывается on-chain, а что нет

Три аудиторских прохода (криптография/безопасность, код/баги, оптимизация). Главный вывод
переформулировал заявление проекта; он вынесен в README и в CLAUDE.md как ограничение 0.

### A1 — криптография и безопасность

**[КРИТИЧНО] Хеш-шаг ML-DSA вне схемы — on-chain доказательство не устанавливает существование
подписи.** Схема V23 доказывает арифметику: `w' = A·z − c·t1·2^d` через NTT/INTT, `‖z‖∞ < γ₁−β`,
восстановление подсказок и границу ω, диапазонные проверки. Шаг FIPS 204
`c̃ = SHAKE-256(μ ‖ w1Encode(w1'))` и `c = SampleInBall(c̃)` **не арифметизирован**. `c̃`
подмешивается в Fiat-Shamir (V19+), но ни одно ограничение не связывает его с `w1'`. Значит,
злоумышленник, выбравший `z` малой нормы, `c` правильной формы и собственный `t1`, удовлетворяет
всем ограничениям без всякой подписи, и верификатор это примет.

Смягчение, которое есть: `extract_mldsa_witness_py` вызывает полный `ml_dsa_verify` и отказывается
извлекать witness из невалидной подписи, поэтому честный агрегатор фальшивку не докажет. Но эта
проверка **вне цепи и контракту не видна** — он доверяет тому, что prover её выполнил.

Закрытие требует SHAKE-256 (Keccak-f[1600]) как AIR. Этого **нет ни в коде, ни в планах**;
термин «Keccak AIR» ввёл я в ходе аудита, а не проектная документация.

**[КРИТИЧНО] Доказывается одна подпись из батча, а не N.** `testnet/e2e.py:206` и
`aggregator/batcher.py:294` берут `batch.transactions[0]`. Остальные транзакции коммитятся
Merkle-корнем, но их подписи не доказываются. Отдельный путь `prove_mldsa_batch` проверяет N
подписей **в Rust** и доказывает только хеш-цепочку по результатам (`lib.rs:5469`), причём
невалидные молча пропускаются (возвращается счётчик `rejected`, но доказательство их не отличает).

**Что проверено и чисто:** секретов в git нет (`.env` в `.gitignore`, в индексе только
`.env.example`); приватные ключи нигде не логируются; bandit по `core/ aggregator/ stark/
sdk/python/ testnet/` — чисто; сравнение Bearer-токена константное (`hmac.compare_digest`);
rate-limiting на всех эндпоинтах; `derive_address` валидирует длину ключа; `wipe_key` через Rust
`zeroize`; on-chain nonce-реестр и off-chain replay guard на месте; текст ошибок `POST /transactions`
не эхоит внутренние детали.

### A2 — код, баги, тупики

**[ВЫСОКОЕ → ИСПРАВЛЕНО 2026-07-31] Продуктовый слой был отключён от стека.**
`aggregator/batcher.py`, `aggregator/api.py`, `sdk/python/qlsa`, `sdk/js` выдавали VFRI7–VFRI10.
Дефолтный стек — v7 (VFRI11), рекурсивный — v8. Хинты VFRI10 верификатором VFRI11 не принимаются
**by design** (другой хеш-бэкенд → другой trace root → другие FRI query indices). До деплоя доходил
только `testnet/e2e.py`.

Причина была в форме, а не в забывчивости: каждый протокол — шесть полей в `BatchResult`,
скопированный блок в `Batcher`, поля ответа в API и типы в двух SDK. Добавить VFRI11 значило
править четыре слоя, поэтому этого просто не сделали, и агрегатор молча перестал попадать в реестр.

**Исправление.** Протокол выбирается по ИМЕНИ: `stark.prover.WITNESS_PROTOCOLS` +
`DEFAULT_WITNESS_PROTOCOL = "vfri11"` (следует дефолтному стеку). Четыре скопированных блока —
один цикл, 24 поля — один словарь `witness_proofs`. `recursive` подключён через адаптер, а не
второй иерархией типов: внешнее доказательство ложится на те же три поля, а inner publics, нужные
только `BatchRegistryV7`, живут в необязательном `WitnessGroup.inner`. Старые имена
(`vfriN_*`, `has_vfriN`) стали read-only видами — существующие клиенты не сломались.

API отдаёт `witness_protocols`, `witness` (протокол → пара коммитментов) и `registry`
(`"direct"` | `"recursive"`) на всех четырёх batch-эндпоинтах; оба SDK — `protocols` /
`commitments`. Попутно найден баг: `_prove_witness_local` при падении VFRI7 возвращал
`has_witness=False` и не пробовал остальные протоколы.

**Проверено функционально при боевой стойкости:** подписанная ML-DSA-65 транзакция через
`Batcher(witness_protocols=("recursive",), n_fri_queries=20)` даёт оба внешних доказательства
(хинты 10976 B / 9760 B), inner publics с `nQueries=20`, `treeDepth=10`, 7 FRI-корнями и последними
слоями 16 и 4 — **130 бит on-chain**, та же форма, что отправляет `--stack v8`.

**[СРЕДНЕЕ] `MAX_SENDERS = 3000` объявлен во всех шести реестрах и недостижим.** O(n²) сканирование
дубликатов даёт практический предел ~200 на `BatchRegistryV5` и меньше на `BatchRegistryV7`
(там сама проверка занимает 13.17M из 16.78M). Превышение даёт OUT OF GAS, а не чистый
`SenderCountExceedsLimit`. `submitBatch` без nonce'ов цикла не содержит и не ограничен.

**[ИСПРАВЛЕНО] Устаревшие утверждения в коде.** Девять мест подавали опровергнутые числа как
текущие; худшее — NatSpec `QLSAVerifierRecursive`: «не влезает on-chain, не стройте на этом
пайплайн» — при том, что `BatchRegistryV7` и весь стек v8 построены именно на нём. Исправлено;
цифры вынесены в `contracts/test/fixtures/measurements.json` и перемеряются
`contracts/test/Measurements.test.js` (падение при расхождении >3%).

### A3 — предложения по оптимизации (не реализованы)

Перечислены в чате аудита; ключевые: убрать O(n²) дедупликацию через требование строго
возрастающего массива `senders` (предел ~212 → ~480, минус квадратичный член); rate-8 squeeze в
`Poseidon2ChannelT16` (~19 перестановок на verify → ~5); батчевая инверсия M31 в генераторах хинтов.

### Воспроизведение

Прогон против отдельного `hardhat node` (настоящий JSON-RPC, не внутрипроцессная EVM) от настоящих
подписей ML-DSA-65: **13 168 471 газа, батч финализирован**. Публичный тестнет из среды разработки
недоступен (егресс закрыт) — остаётся открытым пунктом перед mainnet.


## R4.20–R4.21 (2026-07-30) — стек v8 собран: 130 бит on-chain от подписи до транзакции

Python-обвязка (`gen_mldsa_v23_recursive_bundles`, `prove_mldsa_sig_recursive_stark` через PyO3) и
вся testnet-обвязка: `deploy_v8.js`/`deploy_v8.sh` (VFRI11 + QLSAVerifierRecursive +
BatchRegistryV7), `OnchainSubmitterV7`, `--stack v8`.

**Сквозной прогон на живом узле от настоящих подписей ML-DSA-65: финализировано за 13 235 044 газа
при 130-битной стойкости, одна транзакция.**

Решения, которые стоит знать:

- **`--stack v8` сам поднимает `n_queries` до 20**, если оставлен дефолтный 1: при одном запросе v8
  строго хуже v7, и молчаливый запуск в бессмысленной конфигурации — плохой дефолт.
- **QM31-скаляры пересекают границу Python↔Rust десятичными строками** (это u128); `as_inner_publics()`
  отдаёт ровно набор ключей солидити-структуры `InnerPublics`, набор закреплён тестом.
- **Зонд формы реестра расширен на третий вид.** `crossBoundRoot` есть только у V7 и проверяется
  ПЕРВЫМ: у V7 нет и `pendingGroups`, поэтому при обратном порядке он был бы принят за V4/V5.
- **Nonce из `pending`, а не `latest`** — поймано на живом узле («nonce too low» до вызова).

**598 Python, 1038 Solidity, 524 Rust, 0 падений**, mypy `--strict` и bandit чисто.


## R4.19 (2026-07-30) — 130-битная on-chain стойкость достигнута на НАСТОЯЩИХ данных

Rust-сторона стека v8 собрана: `gen_mldsa_v23_recursive_bundles` выдаёт cross-bound пару
рекурсивных bundle'ов из настоящих V23-групп (1298 и 2206 колонок). LOG=8 извлечён той же схемой
общего билдера колонок, что и LOG=10, — ABI-генератор хинтов и извлекатель входов рекурсии
потребляют ОДНУ реализацию и разъехаться не могут. Инвариант «trace root не зависит от batch root»,
на котором держится двухпроходная привязка, **проверяется** возвратом ошибки, а не предполагается.

**Замер on-chain при боевых 20 запросах (130 бит):** `BatchRegistryV7.submitBatch` финализирует
полный V23-батч за **13 128 561 газа в ОДНОЙ транзакции** (запас 22% под cap 16 777 216). Прямая
проверка того же утверждения при q=20 в транзакцию **не влезает вовсе**.

Цепочка замкнута на реальных данных: **подпись ML-DSA-65 → V23 (3505 колонок) → внутренние VFRI11
при 20 запросах → рекурсия → одна транзакция при 130-битной стойкости.** До этого 130 бит были
недостижимы on-chain в принципе — именно ради этого и строилась рекурсия (см. `docs/conclusions.md`).

**1038 Solidity, 524 Rust, 0 падений, 0 warnings.** Осталось для v8: PyO3 + `stark/prover.py`,
деплой-скрипт, `--stack v8`.


## R4.11–R4.12 (2026-07-30) — привязка `compValue` к `compRoot` закрыта

**R4.11 — снят блокер, и по пути исправлено моё же неверное суждение.** В прошлом отчёте я
рекомендовал обходить препятствие «вариантом 2» (второй Merkle-компонент с namespace-параметризацией
ID), считая, что переменная глубина пути потребует правок AIR. Проверка показала обратное: `depth`
встречается ТОЛЬКО в билдерах и не появляется в eval AIR ни разу — ограничения читают исключительно
preprocessed-селекторы. Значит переменная глубина — вопрос исключительно раскладки слотов, без
единой правки ограничений. Реализовано: `comp_layout(depths)` вместо деления `comp / depth`,
`build_preproc_multi_var`, `compute_log_size_multi_var`. Равномерные функции сохранены как обёртки,
и эквивалентность доказана тестом, сравнивающим ВСЕ preprocessed-столбцы значение за значением.

**R4.12 — привязка закрыта.** Композиция доказывает 3N путей: N финального fold'а (глубина
`tree_depth − num_folds`) + 2N композиционных (глубина `tree_depth`). Смешанные глубины в одном
компоненте — благодаря R4.11.

Проверено **на настоящих данных VFRI11** (синтетика в прошлый раз пропустила ошибку в предпосылке,
поэтому теперь E2E — основная проверка, а не финальная): `comp_depth > depth`, все N путей fold'а
сходятся на генуинном `friLayerRoots[K]`, все 2N comp-путей — на генуинном `compRoot`, доказательство
верифицируется.

Цепочка замкнута с обеих сторон полностью в цепи:
...
**R4.14 — `BatchRegistryV7`.** Реестр, финализирующий батч из РЕКУРСИВНЫХ доказательств. Батч V23 —
две группы, поэтому принимаются два bundle'а с той же cross-proof привязкой, что в V5 (каждый связан
с trace root'ом другой группы). Привязка жёстче, чем в V5: `traceRoot` — явное публичное поле,
покрытое `outerBindingRoot`, а не извлекаемое из сырых байт. Фикстура — настоящая cross-bound пара,
генератор двухпроходный и **проверяет ассертом**, что trace root не зависит от `batchRoot`.
**Измерено: 8 357 379 газа в ОДНОЙ транзакции** (запас ~2× под cap).

Найденное свойство, задокументированное явно: мой же тест ожидал отказа при обмене bundle'ов
местами — отказа нет, пара ограничений симметрична. Тест был неверен, не контракт. Соунднес не
нарушен (дублирование одной группы вместо двух отвергается), но `batchCommitmentsLog10/Log8` —
позиционные метки, а не подтверждённые идентичности групп; у V5 то же свойство. Закреплено тестом.

**R4.15 — замер на PRODUCTION-конфиге: рекурсия сейчас ДОРОЖЕ прямой проверки.** Прежде чем строить
стек v8, измерил рекурсию на реальных параметрах V23 (группы depth 10 и 8, `num_folds=6`). Это
оказалось дёшево сделать, потому что **размер внешнего трейса зависит только от
`(n_queries, num_folds, tree_depth)`, а не от числа колонок внутреннего доказательства**.

| Один и тот же батч V23 | Газ |
|---|---|
| **v7 прямой** (VFRI11 + BatchRegistryV5) | **6 058 052** |
| **рекурсивный** (BatchRegistryV7) | **16 594 419** |

В 2.7 раза дороже, и запас под cap всего **182 797 газа (1.1%)**. Моя же экстраполяция за несколько
минут до замера давала ~9.5M — ошибка в 1.8 раза, второй такой случай за сессию.

Причина: внешнее доказательство — это само VFRI11-доказательство (87 колонок × 1024 строки) со своим
FRI, и при `n_queries=1` амортизировать нечего. Гипотеза (не замер): прямая проверка растёт линейно
по `n_queries`, внешний трейс — логарифмически, поэтому на боевых 20 запросах (130 бит) картина
должна перевернуться.

**R4.16 — точка окупаемости измерена; вывод R4.15 был артефактом настройки.**

| `n_queries` | прямая | рекурсия | отношение |
|---|---|---|---|
...
Окупаемость при **q ≈ 2**. Прямая проверка растёт линейно и упирается в cap на 8 запросах; рекурсия
растёт логарифмически (+0.5M за удвоение).

Сначала измерение гипотезу ОПРОВЕРГАЛО: при `outer_folds=2` рекурсия не влезала в cap уже при q=2.
Причина — on-chain проверка последнего слоя ВНЕШНЕГО доказательства перестраивает дерево из
`2^(outer_log − outer_folds)` листьев, и при фиксированных 2 fold'ах этот член растёт линейно с
внешним трейсом. Масштабирование `outer_folds` (цель — 32 листа) делает его константным.

Это же исказило R4.15: production-замер `BatchRegistryV7` был 16 594 419 (запас 1.1%), после
настройки — **9 204 207** (запас 45%).

**Урок, третий за сессию:** параметр, подобранный для малого масштаба, тихо становится доминирующим
членом на большом.

**R4.17 — замерено при боевом `n_queries=20`** (экстраполяцию не оставляли непроверенной):
прямая проверка перестаёт влезать в транзакцию между 8 и 16 запросами; рекурсия при 16 и 20
запросах стоит одинаково — **6 999 247 газа, запас 58%** (`outer_log`=14 в обоих случаях).

**Итог:** при q=1 рекурсия в ~1.5 раза дороже прямой проверки; при боевых 20 запросах прямая
проверка **невозможна в принципе**, рекурсия стоит 7.0M. Рекурсия — механизм для БОЕВОЙ стойкости,
а не экономия газа. Поднять `n_queries` до 20 и построить стек v8 — это ОДНА задача.
Сводка серии: `docs/conclusions.md`.
