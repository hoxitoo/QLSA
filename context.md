...
Самая серьёзная из оставшихся дыр соунднеса рекурсии. Найдена при оценке готовности
`BatchRegistryV7` — и она же показала, что этот реестр строить пока рано.

**Проблема.** `build_trace` ВЫВОДИТ `compValue = fₚ·(px − z_x) + oodsCombo` из присланного
prover'ом `fₚ` (документация `oods_air` фиксирует это прямо). То есть OODS-соотношение было
тавтологией: любой `fₚ` давал самосогласованный `compValue`, и вся цепочка fold'ов висела на
значении, ничем не связанном с деревом композиции внутреннего доказательства. Prover мог взять
произвольный `fₚ`, вывести под него `compValue` и получить валидное рекурсивное доказательство.

**Исправление.** `QueryChallenges` несёт `comp_pos`/`comp_neg` — значения, которые верификатор
аутентифицировал против `compRoot`; они закреплены в 8 preprocessed-столбцах с ограничениями
равенства под гейтом `is_step`. Направление зависимости инвертируется: `fₚ` полностью определяется
данными верификатора, как в on-chain верификаторе. `RV_PREPROC_COLS` 17 → 25 во всех трёх
композициях (t=2/t=8/t=16).

Регрессия `test_forged_comp_cannot_prove` — с проверкой на неvacuous-ность: подделанный трейс
ДОКАЗЫВАЕТСЯ, если закрепить его собственный `compValue`, что и объясняет, почему пин обязан
приходить от верификатора. 153 recursive-теста, **522 всего**.

**Порядок работ (важно).** До того как рекурсивное доказательство сможет заменить `verify()`
целиком, нужны ещё два элемента: Merkle-путь `hashLeaf(compValue) @ queryIndex → compRoot` в цепи
(входной аналог уже доказываемого пути последнего слоя) и проверка ограниченной степени последнего
слоя. Поэтому **`BatchRegistryV7` пока преждевременен** — он заявлял бы соунднес, которого
рекурсия не даёт. Зафиксировано в `docs/roadmap/recursion.md` § R4.10.

**Попытка сделать первый из двух пунктов и найденное препятствие.** Прямолинейный подход — сложить
comp-пути в тот же multi-path компонент, что и пути финального fold'а — был реализован и проверен:
извлечение comp-путей из `ch.comp_levels` работает, синтетические тесты композиции проходят (154
recursive), но E2E на настоящей VFRI11-цепочке падает с `"all comp paths must share the trace
depth"`. Причина структурная: `compRoot` коммитит по ВСЕЙ области трейса (глубина = `tree_depth`),
а дерево последнего FRI-слоя имеет `2^(tree_depth − num_folds)` листьев (глубина
= `tree_depth − num_folds`) — 4 против 2 в тестовой конфигурации. `merkle_path_t8_air`
предполагает единую глубину для всех путей: она входит в раскладку строк и позиции закреплённых
селекторов. Эксперимент откачен; требуется либо поддержка переменной глубины в AIR, либо второй
Merkle-компонент с параметризованными по неймспейсу ID preprocessed-столбцов. Разобрано в
`docs/roadmap/recursion.md` § R4.10.

**Технический долг** (по решению владельца проекта не трогаем): отставание `aggregator` от
дефолтного стека (нет VFRI11) и O(n²) дедупликация отправителей — оба вынесены в
`docs/roadmap/recursion.md` § «Технический долг» с измерениями и планом.


## Аудит R4.8–R4.9 (2026-07-30) — крипто + код

Двухэкспертный аудит слоя, добавленного в R4.8/R4.9 (перезапись Poseidon2, стек MVP-7,
исправление nonce'ов, зонд формы реестра). Ниже — что проверено, что найдено, что исправлено.

### Крипто: ленивая редукция проверена эмпирически, не «на словах»

Самое рискованное утверждение R4.8 — что линейные слои можно складывать без `mod P`. Если
граница величин занижена, промежуточное значение переполнит uint256 и Solidity молча завернёт
его (`unchecked`), дав неверный хеш. Написана модель на Python, повторяющая Solidity буква в
букву (Python-int неограничен, поэтому переполнение видно как величина выше 2^256), и прогнана
против ПОЛНОСТЬЮ редуцированной референс-реализации:

- 3005 входов, включая максимально достижимые (2^32−1), ровно `P`, `P±1`, нули и случайные;
- **0 расхождений** с референсом;
- пик промежуточных величин: **t=2 → 2^32, t=4 → 2^90, t=8 → 2^75**; запас до 2^256 — 2^165.

Задокументированные в коде границы (2^33 / 2^99 / 2^80) являются корректными ВЕРХНИМИ оценками
(измеренные пики ниже) — то есть аргумент безопасности верен, а не просто правдоподобен.
Достижимый диапазон входов ограничен 2^32 на всех путях вызова: sponge делает `% P`, канал
абсорбирует u32 двумя условными вычитаниями, `_unpack4` маскирует по 0xFFFFFFFF.

### Крипто: канонизация лимбов — проверено, находки нет

Проверено, не принимается ли где-то неканоничный лимб M31 (`v ≥ P`) так, что это меняет решение.
`QLSAVerifierVFRI11` сравнивает вычисленный fold с присланным как СЫРОЙ `uint128`
(`circleFold(...) == h.foldedValue`), что жёстко принуждает `foldedValue` к каноничной форме;
все остальные сравнения идут через QM31-арифметику, которая редуцирует. Абсорбция в канал
(`_absorb`) редуцирует каждое слово, поэтому `v` и `v+P` дают ОДИН И ТОТ ЖЕ транскрипт —
пространства для гриндинга challenge'ей нет. Соунднес не нарушен.

### Крипто/тесты: закрыт пробел в кросс-чеке sponge (padding)

**[НАЙДЕНО И ЗАКРЫТО]** Существовавшие кросс-чеки Poseidon2 sponge покрывали только длину,
кратную rate (n=8 для t=8, n=8 для t=4) — то есть путь БЕЗ padded tail. Ветка нечётного хвоста
(абсорбировать `rem[k]` в `state[k]`, затем `state[7] += 1`) — код, который я в R4.8 переписал
руками — не была закреплена НИ НА ОДНОЙ стороне. Сгенерированы авторитетные векторы из Rust и
зафиксированы с обеих сторон: `sponge_t8` для n=1..12 (все остатки mod 4) и `sponge_t4` для
n=1..8 (обе чётности), плюс проверки «padding разделяет соседние длины» и «v и v+P абсорбируются
одинаково». Расхождений нет — ветка была корректна, но теперь она доказана, а не предположена.
5 тестов Rust + 3 теста Solidity.

### Код: зонд формы реестра проглатывал транспортные ошибки

**[НАЙДЕНО И ИСПРАВЛЕНО — в моём же коде из R4.9]** `detect_registry_kind` ловил
`except Exception`, поэтому сбой RPC (мёртвый эндпоинт, таймаут, rate limit) классифицировался
как «функции нет» → «это V4/V5» — и отправка на BatchRegistryV6 продолжалась, ровно та ошибка,
от которой зонд и должен защищать. Проверено на живом узле, какие исключения реально возникают:
отсутствующий селектор → `web3.exceptions.ContractLogicError`, мёртвый эндпоинт →
`requests.exceptions.ConnectionError`. Обработка сужена до контрактных ошибок
(`ContractLogicError`, `BadFunctionCallOutput`, `DecodingError`); всё остальное пробрасывается.
Оба поведения закреплены тестами.

### Код: `MAX_SENDERS = 3000` недостижим — заявленный предел вводит в заблуждение

**[НАЙДЕНО, ИЗМЕРЕНО, ЗАДОКУМЕНТИРОВАНО]** Проверка дубликатов отправителей в
`submitBatchWithNonces` квадратична, и именно она, а не константа, ограничивает вызов. Замеры на
реальном V23 t=8 батче (`gasUsed` отправленной транзакции):
...
| 50      | 7 599 183 |
| 100     | 9 731 908 |
| 200     | 15 774 846 (всего ~1M до cap) |
| 400     | **out of gas** |

Квадратичный коэффициент ≈201 газа/n²; при n=3000 потребовалось бы **≈1821M газа — в 108 раз
выше cap**. Достижимый максимум ≈212, причём превышение даёт OUT OF GAS, а не чистый
`SenderCountExceedsLimit`. Это реальный потолок пропускной способности: батч с on-chain
защитой от replay несёт не более ~150 отправителей с запасом.

Исправлено честно, без ломки интерфейса: во ВСЕХ пяти реестрах (V2/V3/V4/V5/V6) NatSpec теперь
says прямо, что константа — это backstop, а не заявленная возможность, с полной таблицей замеров
и двумя рекомендациями: использовать `submitBatch` (в нём нет цикла по отправителям, размер батча
не ограничен) когда on-chain nonce'ы не нужны; либо потребовать строго возрастающий `senders`,
тогда дубликаты невозможны, внутренний цикл исчезает и предел определяется записью в storage
(~22k/отправитель → n≈480). Второе — изменение интерфейса (вызывающий обязан сортировать) и
СОЗНАТЕЛЬНО не сделано в этом проходе. Добавлен регрессионный тест на 100 отправителей.

### Код: CI-падение, вызванное моей же ошибкой процесса

**[НАЙДЕНО И ИСПРАВЛЕНО]** `tests/test_testnet_registry_kind.py` импортирует `testnet.submit`,
которому нужен `web3` — а он лежит в `requirements-testnet.txt`, который CI не устанавливает.
Локально тест проходил только потому, что я поставил web3 вручную. Классика «работает на моей
машине, потому что я загрязнил окружение». Исправлено через `-r requirements-testnet.txt` в
`requirements-dev.txt` (не дублирующим пином — чтобы у версии был ОДИН источник истины).

### Код: гигиена предупреждений

Заявление «0 warnings» верно для **release**-сборки (проверено: 0), но в тестовом профиле было
12 предупреждений (неиспользуемые импорты в `#[cfg(test)]`, неиспользуемая переменная, мёртвый
тестовый хелпер) — они маскировали бы новые сигналы. Устранены все; 521 тест Rust проходит и
тестовый профиль теперь тоже чист. Отдельно отмечаю: моё более раннее заявление «0 warnings» в
этой сессии опиралось на усечённый лог (`tail -20`), то есть было неверным.

### Проверено чисто (находок нет)

- Секретов в отслеживаемых файлах нет; `.env` и `.env.deployed` в `.gitignore`; `deploy_v7.sh`
  создаёт `.env.deployed` с `umask 077`.
- Широкие `except Exception` в `aggregator/batcher.py` — намеренные (восстановление после падения
  prover'а), логируют с `exc_info` и выставляют флаг деградации. Не находка.
- HTTP-клиент SDK имеет таймаут 30 с.
- Парсинг адресов в `deploy_v7.sh` однозначен (проверено на смоделированном выводе: ровно одно
  совпадение на каждый адрес, несмотря на повтор адресов в summary-блоке).
- Отображение nonce `n → n+1` инъективно и сохраняет строгую монотонность, поэтому гарантия
  защиты от replay не ослаблена.

### Известное расхождение (не исправлено, требует решения)

`aggregator/` умеет VFRI7/8/9/10, но НЕ VFRI11 — то есть отстаёт от стека, который стал
дефолтным. Witness-доказательства агрегатора нельзя отправить в реестр v7. Это не баг, а
незавершённая интеграция: затрагивает `Batcher`, HTTP API и оба SDK (Python + JS), поэтому
выделено как отдельная задача, а не сделано внутри аудиторского прохода.
...
| Путь | До | После |
|------|-----|-------|
| `t8.permute` / `t4.permute` (harness) | 127 228 / 61 331 | **36 251 / 28 149** |
| `t2.permute` (harness, для единообразия) | 25 509 | **23 008** |
...
### R4.9 — t=8 стал стеком по умолчанию (2026-07-30)

**`--stack v7` теперь дефолт.** Это сильнейшая доступная on-chain стойкость (узел ~2^62 против ~2^31
у t=4), атомарная (одна транзакция) и с запасом под cap (6.06M из 16.78M). `--stack v6` (t=4, split
на 2 tx) остаётся осмысленным выбором, когда важнее меньший пик газа на транзакцию: 2.15M + 1.70M.

**Защита от несовпадения формы реестра.** Смена дефолта повышает риск, что оператор с ранее
развёрнутым `BatchRegistryV6` запустит e2e без флагов и попадёт селектором в контракт без такой
функции — с невнятной ошибкой внутри транзакции. Добавлен зонд: `pendingGroups(bytes32)` есть ТОЛЬКО
у V6 → точный дискриминатор двух форм. `detect_registry_kind` / `require_registry_kind` проверяют
форму до отправки и говорят, что именно не совпало (+ отдельно ловят «по адресу нет контракта» —
самую частую ошибку конфигурации). V4 против V5 on-chain неразличимы (ABI побитово идентичны), но это
несовпадение и так проявляется читаемо — `Log10ProofInvalid`. 8 тестов.

**Poseidon2 t=2 приведён к общему виду.** Продакшн-выигрыша нет (t=2 живёт только в VFRI8/VFRI9,
которые по ограничению #7 не деплоятся). Причина — единообразие: после R4.8 t=2 оставался
единственной библиотекой с ветвистым `if (r >= P) r -= P`, то есть образцом анти-паттерна для нового
кода. Замер это подтвердил: по газу на «ячейку×раунд» t=2 был ХУДШИМ из трёх (~250 против ~57 у t=4
и ~84 у t=8). Та же ленивая редукция: 25 509 → 23 008; все 17 кросс-чек-тестов против замороженных
Rust-векторов проходят без изменений.

Исправлена неточность R4.8: `t4.permute` = **28 149** (прямой замер), а не «~16 000» — та цифра была
оценкой из падения газа групповой проверки. По самой перестановке это ≈6 600 против ≈40 000, то есть −6×.
