...
/// @title QLSAVerifierVFRI11 — VFRI10 protocol on the Poseidon2 t=8 hash backend
///
/// Identical proof protocol, ABI, and last-layer FRI check as VFRI10; the only
/// change is the hash backend, swapped from the t=4 Poseidon2 primitives to
/// their t=8 successors:
///
///   Poseidon2MerkleVerifierT4 → Poseidon2MerkleVerifierT8   (Merkle)
///   Poseidon2ChannelT4        → Poseidon2ChannelT8          (Fiat-Shamir)
///
/// The queryHints ABI is byte-compatible with VFRI9/VFRI10 (Merkle siblings
/// are still bytes32; only the node *contents* widen from 2 to 4 M31 words).
/// The t=8 permutation runs over a 248-bit state, so a 2-to-1 compression
/// carries 4-word (124-bit) nodes — node/transcript collision cost rises from
/// t=4's ~2^31 (2-word nodes) to ~2^62.  Next rung toward 128-bit on-chain
/// binding (documented limitation #6); t=16 reaches the full target.
///
...
///
/// 2. WIDE MERKLE NODES via Poseidon2MerkleVerifierT8 — FOUR sponge words per
///    node ((w0<<96)|(w1<<64)|(w2<<32)|w3), hashed by the t=8 permutation.
///
...
///    of only the low 4 bytes; wide node roots use 4 words (mixRootW).
///
/// Proof version marker: proof[0:8] = 5 (little-endian; VFRI10 uses 4).
///
/// Transcript:
///   Poseidon2ChannelT8.mixRootFull(traceRoot)           ← proof[8:40], 8 words
///   z_x       = drawSecureFelt
...
contract QLSAVerifierVFRI11 is IQLSAVerifierV4 {
    ...
    function _buildCtx(
        ...
        Poseidon2ChannelT8.State memory chan = Poseidon2ChannelT8.init();
        Poseidon2ChannelT8.mixRootFull(chan, embeddedRoot);

        ctx.z_x       = Poseidon2ChannelT8.drawSecureFelt(chan);
        ctx.compAlpha  = Poseidon2ChannelT8.drawSecureFelt(chan);

        {
            ...
            Poseidon2ChannelT8.mixU32s(chan, comboWords);
        }

        Poseidon2ChannelT8.mixRootW(chan, compRoot);
        ctx.friAlpha = Poseidon2ChannelT8.drawSecureFelt(chan);
        Poseidon2ChannelT8.mixRootW(chan, friLayerRoots[0]);

        uint256 numFolds = friLayerRoots.length - 1;
        ctx.friAlphas = new uint128[](numFolds);
        for (uint256 k = 0; k < numFolds; k++) {
            ctx.friAlphas[k] = Poseidon2ChannelT8.drawSecureFelt(chan);
            Poseidon2ChannelT8.mixRootW(chan, friLayerRoots[k + 1]);
        }

        // Cross-proof binding: mix the FULL merkleRoot before drawQueries.
        Poseidon2ChannelT8.mixRootFull(chan, merkleRoot);

        ctx.derivedIndices = Poseidon2ChannelT8.drawQueries(chan, logDomainSize, nQueries);
    }
    ...
    function _verifyQuery(
        ...
        if (!Poseidon2MerkleVerifierT8.verifyMem(
            ctx.compRoot,
            Poseidon2MerkleVerifierT8.hashLeaf(_qm31ToWords(h.compValue)),
...

        {
            ...
            if (!Poseidon2MerkleVerifierT8.verifyMem(
                ctx.compRoot,
                Poseidon2MerkleVerifierT8.hashLeaf(_qm31ToWords(h.compValueNeg)),
        ...
        if (!Poseidon2MerkleVerifierT8.verifyMem(
            ctx.friLayerRoots[0],
            Poseidon2MerkleVerifierT8.hashLeaf(_qm31ToWords(h.foldedValue)),
    ...
    function _verifyFolds(
        ...
        for (uint256 k = 0; k < numFolds; k++) {
            ...
            if (!Poseidon2MerkleVerifierT8.verifyMem(
                ctx.friLayerRoots[k],
                Poseidon2MerkleVerifierT8.hashLeaf(_qm31ToWords(h.folds[k].siblingValue)),
            ...
            if (!Poseidon2MerkleVerifierT8.verifyMem(
                ...
                Poseidon2MerkleVerifierT8.hashLeaf(_qm31ToWords(h.folds[k].foldedValue)),
