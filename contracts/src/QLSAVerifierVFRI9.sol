...
import "./verifier/Poseidon2MerkleVerifierW.sol";
import "./verifier/Poseidon2Channel.sol";
import "./verifier/CirclePoint.sol";

/// @title QLSAVerifierVFRI9 — VFRI8 + last-layer FRI check + wide Poseidon2 nodes
///
/// Three security upgrades over VFRI8:
///
...
///
/// 2. WIDE MERKLE NODES (62-bit): Poseidon2MerkleVerifierW carries both sponge
///    words per node ((s0 << 32) | s1) instead of VFRI8's single 31-bit word,
///    raising node collision cost from ~2^15.5 to ~2^31 (t=2 maximum; 128-bit
///    binding requires t>=4 / RPO256 — documented limitation).
///
...
///
/// Transcript:
///   Poseidon2Channel.mixRootFull(traceRoot)           ← proof[8:40], 8 words
///   z_x       = drawSecureFelt
...
contract QLSAVerifierVFRI9 is IQLSAVerifierV4 {
    ...
    function _checkLastLayer(
        ...
        for (uint256 i = 0; i < lastLayerSize; i++) {
            nodes[i] = Poseidon2MerkleVerifierW.hashLeaf(_qm31ToWords(evals[i]));
        }
        uint256 sz = lastLayerSize;
        while (sz > 1) {
            sz >>= 1;
            for (uint256 i = 0; i < sz; i++) {
                nodes[i] = Poseidon2MerkleVerifierW.hashPair(nodes[2 * i], nodes[2 * i + 1]);
            }
        }
    ...
    function _buildCtx(
        ...
        Poseidon2Channel.mixRootFull(chan, embeddedRoot);
    ...
    function _verifyQuery(
        ...
        if (!Poseidon2MerkleVerifierW.verifyMem(
            ctx.compRoot,
            Poseidon2MerkleVerifierW.hashLeaf(_qm31ToWords(h.compValue)),
...

        {
            ...
            if (!Poseidon2MerkleVerifierW.verifyMem(
                ctx.compRoot,
                Poseidon2MerkleVerifierW.hashLeaf(_qm31ToWords(h.compValueNeg)),
        ...
        if (!Poseidon2MerkleVerifierW.verifyMem(
            ctx.friLayerRoots[0],
            Poseidon2MerkleVerifierW.hashLeaf(_qm31ToWords(h.foldedValue)),
    ...
    function _verifyFolds(
        ...
        for (uint256 k = 0; k < numFolds; k++) {
            ...
            if (!Poseidon2MerkleVerifierW.verifyMem(
                ctx.friLayerRoots[k],
                Poseidon2MerkleVerifierW.hashLeaf(_qm31ToWords(h.folds[k].siblingValue)),
            ...
            if (!Poseidon2MerkleVerifierW.verifyMem(
                ...
                Poseidon2MerkleVerifierW.hashLeaf(_qm31ToWords(h.folds[k].foldedValue)),
