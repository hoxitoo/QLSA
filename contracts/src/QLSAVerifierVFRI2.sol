...
/// @title QLSAVerifierVFRI2 — VFRI + last-layer constant-polynomial check
///
/// Extends QLSAVerifierVFRI by verifying that after K fold rounds the FRI
/// polynomial collapses to a degree-0 constant, closing the remaining soundness
/// gap in the on-chain FRI protocol.
///
/// Last-layer check (new in VFRI2):
///   Prover supplies a global `lastLayerValue` (a QM31 element).
///   Verifier reconstructs the expected Merkle root of a constant tree of
///   depth (treeDepth − K) whose every leaf equals hashLeaf(qm31Words(c)):
///     node = hashLeaf(qm31Words(c))
///     repeat (treeDepth − K) times: node = hashPair(node, node)
///   Then asserts: friLayerRoots[K] == node
///
///   Because each query already Merkle-proves its final fold value into
///   friLayerRoots[K], and the constant-tree check proves that every leaf
///   of friLayerRoots[K] equals hashLeaf(c), the final fold value for every
///   query is cryptographically bound to c — completing the FRI argument.
///
/// queryHints ABI encoding (extends VFRI):
///   abi.encode(uint128 lastLayerValue,
///              uint128[] oodsEvalsPos, uint128[] oodsEvalsNeg,
///              bytes32[] friLayerRoots, QueryHints[])
///
/// All other structures (FoldHint, QueryHints, channel transcript) are
/// identical to QLSAVerifierVFRI.
contract QLSAVerifierVFRI2 is IQLSAVerifierV4 {
    ...
    function verify(
        ...
        // ── Last-layer constant-polynomial check (new in VFRI2) ───────────────
        // Reconstruct the expected Merkle root for a constant tree of depth
        // (logDomainSize − numFolds) where every leaf = hashLeaf(qm31Words(c)).
        {
            uint256 lastDepth = logDomainSize - numFolds;
            bytes32 node = MerkleVerifier.hashLeaf(_qm31ToWords(lastLayerValue));
