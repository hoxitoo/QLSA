...
| **Recursion composition — first multi-gadget recursive proof** — `recursive/composition.rs`: `recursive_verifier` + `merkle_path` in ONE multi-component STARK (shared allocator, combined pinned Tree 0). Proves per-query fold chain → `hashLeaf(finalFold)` → Merkle-authenticated into root, value-bound end-to-end. Merkle `leaf` also bound in-circuit (C1). 99 recursive tests | ✅ Done (2026-06-17) |
