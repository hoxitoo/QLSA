...
| **C2 pinning ported — 4 recursion sub-gadgets + 5 production V23 verifiers** — sub-gadgets (`merkle_path`/`channel`/`transcript_draw`/`fri_fold_chain`) each pin via `build_preproc`; `lib.rs` `is_init_uh` verifiers (`verify_use_hint_batch_v2`, Norm+UseHint, Az+Ct1+Norm+UseHint, V21/V22, **V23**) pin via `canonical_uh_preproc_root(max_log)`. Forged `is_init_uh` (could bypass OMEGA hint-weight bound) rejected; honest V21/V22/V23 roundtrips pass | ✅ Done (2026-06-17) |
