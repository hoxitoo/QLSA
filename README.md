...
| **Recursion — FRI cherry-pick closed for fold challenge** — key design realization: the cheap Poseidon2 channel stays on-chain, so challenges are public inputs to the recursive proof → **no logup needed**. `recursive_verifier` pins the fold challenge `alpha` (via `alpha_p` preproc + `alpha − alpha_p = 0` constraint); a prover can't cherry-pick the FRI folding challenge. 102 recursive tests | ✅ Done (2026-06-17) |
