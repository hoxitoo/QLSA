...
**⚠ Recursion audit (2026-06-17):** a two-expert audit (crypto + code) confirmed the gadgets are correct *relation* provers but the composition is **not yet sound against a malicious prover** — two confirmed soundness gaps block R3.7/production wiring: **[C1]** public outputs (`root`/`finalFold`/`digest`) are bound only via Fiat-Shamir, not an in-circuit constraint; **[C2, reproduced]** preprocessed selectors/round-constants are prover-supplied and unpinned (a forged `is_step≡0` + corrupted `compPos` verifies `true`). The same unpinned-preprocessed pattern exists in the mature V23/VFRI verifiers — a per-circuit follow-up review item. Robustness fixes applied in-audit: input caps (`MAX_QUERIES`/`MAX_NUM_FOLDS`), empty-input guard, `bits_to_index` depth assert, brittle-test hardening. See `docs/roadmap/recursion.md` § R3.7 and `stark_stwo/src/recursive/mod.rs`. 437 Rust (+88 recursive) + Solidity (+24 t=8 backend) tests green.
...
| **⚠ Recursion audit — 2 confirmed soundness blockers (R3.7)** — [C1] public outputs bound only via Fiat-Shamir (no in-circuit constraint); [C2, reproduced] preprocessed selectors/constants prover-supplied & unpinned (forged `is_step` verifies). Same pattern in mature V23/VFRI — per-circuit follow-up. Robustness fixes applied (caps, guards, asserts). Must close C1/C2 before production wiring | ⏳ Blocker |
