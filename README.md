...
**Recursion R3.1 in progress** (2026-06-17). The Poseidon2 ladder t=2/t=4/t=8 is complete (`QLSAVerifierVFRI11`, t=8 → ~2^62 node collision), but full-V23 t=8 on-chain `verify()` exceeds 100M gas at depth-10. **Decision: skip standalone t=16 verifier (VFRI12); go straight to proof recursion** — a STARK proving "I verified a VFRI11 STARK" gives constant ~5M on-chain gas with any inner hash width. **Six AIR gadgets** are now complete across R0.1–R3.1: QM31-mul (load-bearing primitive), FRI circle/line fold, OODS quotient, Poseidon2 Merkle auth-path, Poseidon2 Fiat-Shamir transcript sponge, and the first **composition gadget** (per-query FRI step — chains OODS± + circle fold in one row). Each gadget carries a full Stwo prove/verify roundtrip + rejection harness. A two-expert audit found **no Critical/High/Medium**; fixes: `deploy_v6.sh --network` flag, CLI arg validation, web3 HTTP timeouts, `.env.deployed` 0600, t=8 sponge input reduction, QM31 limb-canonicity preconditions. See `docs/roadmap/recursion.md`. 437 Rust (+50 recursive) + Solidity (+24 t=8 backend) tests green.
