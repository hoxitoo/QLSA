...
| **Recursion R3.16 — N-query wide composition (VFRI11 shape on t=8)** — `prove_queries_membership_t8` proves N fold chains + N wide (4-word-node) Merkle paths in ONE STARK via new multi-path t=8 builders (AIR unchanged). Per-query leaves recomputed + pinned, every path root pinned in-circuit, input caps from the start. Next: t=16 (128-bit), on-chain integration. 127 recursive tests | ✅ Done (2026-07-16) |
