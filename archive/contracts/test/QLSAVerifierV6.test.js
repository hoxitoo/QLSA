...
// ─────────────────────────────────────────────────────────────────────────────
// QLSAVerifierV6 — Fiat-Shamir FRI query derivation via TwoChannel
//
// Query indices are derived on-chain from the embedded trace root.
// The JS fixture must first compute derived indices, then build hints for them.
// ─────────────────────────────────────────────────────────────────────────────
...
// Matches TwoChannel.sol + Stwo Rust Blake2sM31Channel exactly.
...
function blake2sM31(buf) {
    ...
    for (let i = 0; i < 8; i++) {
        out.writeUInt32LE(reduceM31(h.readUInt32LE(i * 4)), i * 4);
    }
    return out;
}
...
// ── QueryHints encoding (identical struct layout to V5) ───────────────────────
...
function buildQueryHint(colA, colB, levels, treeDepth, queryIdx, friAlpha) {
    ...
    return { traceRoot, queryValues, queryIdx, treeDepth, siblings, friAlpha,
             fPlus, fMinus, foldedValue, qpX, qpY };
}

// ── V6 fixture builder ─────────────────────────────────────────────────────────
//
// Key difference from V5: query indices are NOT chosen freely.
// We must first build the tree, then run the JS channel to find which indices
// the on-chain verifier will derive, and only then build the Merkle hints.

function buildV6Fixture() {
    // 8-leaf tree (treeDepth = 3, domain size = 8).
    ...
    // Run JS channel to discover which indices V6 will derive.
    const chan = channelInit();
    channelMixRoot(chan, Buffer.from(traceRoot.slice(2), "hex"));
    const nQueries = 2;
    const derivedIndices = channelDrawQueries(chan, treeDepth, nQueries);

    const friAlpha = qm31fromM31(7777n);

    // Build Merkle hints for the channel-derived indices.
    const hints = derivedIndices.map(idx =>
        buildQueryHint(colA, colB, levels, treeDepth, idx, friAlpha)
    );

    // Batch Merkle root (separate from trace root) and commitment binding.
...
describe("QLSAVerifierV6", function () {
    let verifier;
    let f;

    before(async function () {
        const F = await ethers.getContractFactory("QLSAVerifierV6");
    ...
    it("accepts valid 2-query batch with channel-derived indices", async function () {
    ...
    // ── Fiat-Shamir index enforcement ─────────────────────────────────────────

    it("rejects when hint provides a non-derived queryIndex", async function () {
        // Flip queryIdx to a position not in derivedIndices.
        const wrongIdx = (f.derivedIndices[0] + 1) % (1 << f.treeDepth);
        const bad = buildQueryHint(f.colA, f.colB, f.levels, f.treeDepth, wrongIdx, f.friAlpha);
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
    ...
    it("rejects when 2-query hints have correct indices in wrong order", async function () {
        // Swap the two hints; if both derived indices are distinct this fails
        // because hints[0].queryIndex != derivedIdx[0].
        // When both derived indices are equal the test reduces to the same index
        // check — still valid coverage.
        const swapped = [f.hints[1], f.hints[0]];
        const correctOrder = f.derivedIndices[0] === f.derivedIndices[1];
        const result = await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHintsArray(swapped)
        );
        // If indices are distinct, swap must fail; if equal, it still passes.
        expect(result).to.equal(correctOrder || f.derivedIndices[0] === f.derivedIndices[1]);
    });

    it("rejects when queryIndex is supplied as 0 for a non-zero derived index", async function () {
        // Overwrite only queryIdx; Merkle path and fold are for the original index.
        // This tests that the index check fires before the Merkle check.
        if (f.derivedIndices[0] === 0) {
            // derivedIndices[0] happens to be 0 — test the second slot instead.
            if (f.derivedIndices[1] !== 0) {
                const bad = { ...f.hints[1], queryIdx: 0 };
                expect(await verifier.verify(
                    f.proof, f.commitment, f.merkleRoot,
                    encodeHintsArray([f.hints[0], bad])
                )).to.be.false;
            } else {
                this.skip(); // both happen to be 0, skip this variant
            }
        } else {
            const bad = { ...f.hints[0], queryIdx: 0 };
            expect(await verifier.verify(
                f.proof, f.commitment, f.merkleRoot,
                encodeHintsSingle(bad)
            )).to.be.false;
    ...
    it("rejects hints with mismatched treeDepths", async function () {
        const hint0 = f.hints[0];
        const hint1 = { ...f.hints[1], treeDepth: f.treeDepth + 1 };
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHintsArray([hint0, hint1])
        )).to.be.false;
    });
    ...
    it("rejects valid-circle point that is not cosetAt(treeDepth, queryIndex)", async function () {
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
    ...
    it("rejects 2-query batch when second query has wrong circle fold", async function () {
    ...
    // ── Proof bytes with wrong embedded traceRoot ─────────────────────────────

    it("rejects proof with different embedded root (hints valid for original root)", async function () {
        // Build a proof with a different root at bytes[8:40]; hints remain
        // built for f.traceRoot, so traceRoot check fails inside _verifyQuery.
        const altProof = Buffer.from(f.proof.slice(2), "hex");
        Buffer.from("cc".repeat(32), "hex").copy(altProof, 8);
        // Recompute commitment so commitment check passes.
