...
// ─────────────────────────────────────────────────────────────────────────────
// QLSAVerifierV11 — FRI layer 2: line fold + second Merkle decommitment
//
// New in V11 vs V10:
//   1. After friLayer1Root is committed, channel draws friAlpha2 (line fold α).
//   2. Prover computes lineFolded[j] = lineFold(G(x_j), G(−x_j), friAlpha2, xInv_j)
//      for j in [0, N/2), where G(x_j) = foldedValue[j] and G(−x_j) = foldedValue[j+N/2].
//   3. friLayer2Root is the Merkle root of the N/2 lineFolded values.
//   4. Channel absorbs friLayer2Root before drawing queries.
//   5. Per-query: sibling foldedValue in FRI L1, line fold, and FRI L2 are all verified.
//
// V11 transcript order:
//   mixRoot(traceRoot) → z_x → mixU32s(oodsPos) → mixU32s(oodsNeg) → compAlpha → friAlpha
//   → mixRoot(friLayer1Root) → friAlpha2 → mixRoot(friLayer2Root) → drawQueries
// ─────────────────────────────────────────────────────────────────────────────
...
// lineFold uses identical formula but with xInv instead of yInv
...
// ── V11 hint encoding (18-field struct) ───────────────────────────────────────

const HINT_TUPLE = "tuple(bytes32,uint32[],uint32[],uint256,uint256,bytes32[],bytes32[],uint128,uint128,uint128,uint128,uint256,uint256,bytes32[],uint128,bytes32[],uint128,bytes32[])";
...
function encodeHints(hints, oodsPos, oodsNeg, friL1Root, friL2Root, overrides = {}) {
    const tuples = hints.map(h => hintTuple({ ...h, ...overrides }));
    return ethers.AbiCoder.defaultAbiCoder().encode(
        ["uint128[]", "uint128[]", "bytes32", "bytes32", HINT_TUPLE + "[]"],
        [oodsPos.map(v => v), oodsNeg.map(v => v), friL1Root, friL2Root, tuples]
    );
}
function encodeSingle(hint, oodsPos, oodsNeg, friL1Root, friL2Root, overrides = {}) {
    return encodeHints([hint], oodsPos, oodsNeg, friL1Root, friL2Root, overrides);
}

// ── V11 fixture builder ────────────────────────────────────────────────────────
//
// Extends V10 by:
//  6. After friLayer1Root: draw friAlpha2 from channel.
//  7. Compute lineFolded[j] for all j in [0, N/2) using G(x_j) and G(−x_j).
//  8. Build FRI layer 2 tree (N/2 leaves, depth treeDepth−1).
//  9. Mix friLayer2Root → drawQueries.
// 10. Per query: add sibling foldedValue + proof + lineFoldedValue + proof.
...
function buildV11Fixture() {
    ...
    // ── Full-domain circle fold (same as V10) ──────────────────────────────
    const allData = [];
    ...
    const friAlpha2 = chFelt(chan);   // line fold challenge

    // ── Line fold for all N/2 line positions ──────────────────────────────
    // lineFolded[j] = lineFold(G(x_j), G(−x_j), friAlpha2, inv(x_j))
    // where G(x_j) = foldedValue[j] and G(−x_j) = foldedValue[j + half].
    const allLineFolded = [];
    ...
    // ── FRI layer 2 tree (N/2 leaves, depth treeDepth−1) ──────────────────
    ...
    // ── Channel: mixRoot(friLayer2Root) → drawQueries ─────────────────────
    ...
    return {
        ...
        z_x, compAlpha, friAlpha, friAlpha2,
        oodsComboPos, oodsComboNeg,
        derivedIndices, hints, allData, allLineFolded,
...
describe("QLSAVerifierV11", function () {
    let verifier;
    let f;

    before(async function () {
        const F = await ethers.getContractFactory("QLSAVerifierV11");
    ...
    it("accepts valid 2-query batch with FRI layer 2 decommitment", async function () {
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints(f.hints, f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.true;
    });
    ...
    it("accepts valid 1-query batch (first derived index)", async function () {
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeSingle(f.hints[0], f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.true;
    });

    // ── FRI layer 2 enforcement ───────────────────────────────────────────────

    it("rejects zero friLayer2Root", async function () {
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints(f.hints, f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, "0x"+"00".repeat(32))
        )).to.be.false;
    });

    it("rejects tampered lineFoldedValue (line fold check fails)", async function () {
    ...
    it("rejects wrong friL2Siblings (FRI layer 2 Merkle fails)", async function () {
        const badSibs = f.hints[0].friL2Siblings.map(() => "0x"+"dd".repeat(32));
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeSingle(f.hints[0], f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root,
                         { friL2Siblings: badSibs })
        )).to.be.false;
    });

    it("rejects wrong friLayer2Root (changes query indices → mismatch)", async function () {
        const altL2Root = "0x"+"cc".repeat(32);
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints(f.hints, f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, altL2Root)
        )).to.be.false;
    });

    // ── FRI layer 1 sibling enforcement ──────────────────────────────────────

    it("rejects tampered friL1SiblingValue (line fold mismatches)", async function () {
    ...
    it("rejects wrong friL1SiblingProof (sibling Merkle proof fails)", async function () {
        const badProof = f.hints[0].friL1SiblingProof.map(() => "0x"+"ee".repeat(32));
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeSingle(f.hints[0], f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root,
    ...
    it("rejects tampered oodsEvalsPos (channel changes → OODS fails)", async function () {
        ...
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints(f.hints, badPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.false;
    });
    ...
    it("rejects tampered foldedValue (FRI L1 Merkle fails + line fold fails)", async function () {
    ...
    // ── FRI layer 1 root enforcement ──────────────────────────────────────────

    it("rejects zero friLayer1Root", async function () {
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints(f.hints, f.oodsEvalsPos, f.oodsEvalsNeg, "0x"+"00".repeat(32), f.friLayer2Root)
        )).to.be.false;
    });

    it("rejects wrong friLayer1Root (friAlpha2 changes → line fold fails)", async function () {
        const altL1Root = "0x"+"bb".repeat(32);
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints(f.hints, f.oodsEvalsPos, f.oodsEvalsNeg, altL1Root, f.friLayer2Root)
        )).to.be.false;
    });
    ...
    it("rejects non-derived queryIndex", async function () {
        ...
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeSingle(bad, f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.false;
    });

    it("rejects treeDepth < 2", async function () {
        const h1 = { ...f.hints[0], treeDepth: 1 };
        const h2 = { ...f.hints[1], treeDepth: 1 };
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints([h1, h2], f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.false;
    });
    ...
    it("rejects hints with mismatched treeDepths", async function () {
        ...
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints([f.hints[0], bad], f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.false;
    });
    ...
    it("rejects empty hints array", async function () {
        const encoded = ethers.AbiCoder.defaultAbiCoder().encode(
            ["uint128[]", "uint128[]", "bytes32", "bytes32", HINT_TUPLE + "[]"],
            [f.oodsEvalsPos.map(v=>v), f.oodsEvalsNeg.map(v=>v), f.friLayer1Root, f.friLayer2Root, []]
        );
    ...
    it("rejects 2-query batch when second hint has wrong lineFoldedValue", async function () {
        const wrong = (BigInt(f.hints[1].lineFoldedValue) + 1n) % (1n << 128n);
        const bad   = { ...f.hints[1], lineFoldedValue: wrong };
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints([f.hints[0], bad], f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.false;
    ...
    it("rejects 2-query batch when second hint has wrong friL2Siblings", async function () {
        const badSibs = f.hints[1].friL2Siblings.map(() => "0x"+"ff".repeat(32));
        const bad     = { ...f.hints[1], friL2Siblings: badSibs };
        expect(await verifier.verify(
            f.proof, f.commitment, f.merkleRoot,
            encodeHints([f.hints[0], bad], f.oodsEvalsPos, f.oodsEvalsNeg, f.friLayer1Root, f.friLayer2Root)
        )).to.be.false;
