...
/// @title QLSAVerifierVFRI3 — VFRI with non-constant last-layer polynomial check (MVP-4)
///
/// Extends QLSAVerifierVFRI2 by supporting arbitrary low-degree last-layer polynomials,
/// closing the remaining FRI soundness gap (non-constant last layer / bounded-degree check).
///
/// Last-layer check (MVP-4):
///   Prover supplies `lastLayerCoeffs: uint128[]` — the evaluations of the last-layer
///   polynomial at every point of its domain (2^(treeDepth − K) elements, in natural order).
///   Verifier builds the Merkle tree:
///     leaf[i] = hashLeaf(qm31Words(lastLayerCoeffs[i]))
///     tree built bottom-up with hashPair(left, right)
///   Asserts: tree root == friLayerRoots[K]
///
///   Special case: if lastLayerCoeffs.length == 1, uses the constant-tree
///   optimization from VFRI2 (all leaves equal, same value), which is cheaper.
///
///   Because each query already Merkle-proves its final fold value into
///   friLayerRoots[K], and we verify friLayerRoots[K] is the Merkle root of
///   lastLayerCoeffs, the final fold value for each query is cryptographically
///   bound to a specific evaluation of the last-layer polynomial — completing
///   the FRI bounded-degree argument.
///
...
///
/// All other structures (FoldHint, QueryHints, VerifyCtx, transcript) are
/// identical to QLSAVerifierVFRI2.
contract QLSAVerifierVFRI3 is IQLSAVerifierV4 {
