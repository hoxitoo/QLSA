...
**Остаётся до замены `verify()` целиком:** проверка ограниченной степени последнего слоя
(перестроить дерево последнего FRI-слоя из evals и сверить с `friLayerRoots[K]`). После неё
`BatchRegistryV7` становится осмысленным.
