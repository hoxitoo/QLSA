...
**Блокер снят.** Следующий заход — довести привязку `compValue` до конца: comp-пути (глубина
`tree_depth`) и пути финального fold'а (глубина `tree_depth − num_folds`) теперь могут жить в одном
компоненте.
