...
| VFRI11 (t=8) | 2^62 | **3 342 623 / 2 633 375** (V23 LOG=10 / LOG=8) | полный V23 verify==true; дуальный submitBatch 6 058 052 в ОДНОЙ tx |
...
#### R4.21 — стек v8 собран и проверен сквозным прогоном

`contracts/scripts/deploy_v8.js` (VFRI11 + QLSAVerifierRecursive + BatchRegistryV7),
`testnet/deploy_v8.sh`, `testnet.submit.OnchainSubmitterV7`, `--stack v8` в `testnet/e2e.py`.

**Сквозной прогон на живом узле от настоящих подписей ML-DSA-65:**
финализировано за **13 235 044 газа при 130-битной стойкости**, одна транзакция.

Решения по ходу:

- **`--stack v8` поднимает `n_queries` до 20 автоматически**, если оставлен дефолтный 1. При одном
  запросе v8 строго хуже v7, так что молча запускать его в бессмысленной конфигурации — плохой
  выбор по умолчанию. Переопределяется явным `--n-queries`.
- **Зонд формы реестра расширен на третий вид.** `crossBoundRoot(bytes32,bytes32)` есть ТОЛЬКО у
  V7, и проверяется он ПЕРВЫМ: у V7 нет и `pendingGroups`, поэтому при обратном порядке он был бы
  принят за V4/V5 и отправка v8 пошла бы в реестр, который её не обслужит. Порядок закреплён
  отдельным тестом с объяснением.
- **Nonce берётся из `pending`, а не `latest`.** Поймано на живом узле: между чтением счётчика и
  отправкой аккаунт успевал намайнить транзакции, и отправка падала с «nonce too low» ещё до вызова.

14 тестов зонда (было 10), 5 тестов bundle'ов. **598 Python, 1038 Solidity, 524 Rust, 0 падений**,
mypy `--strict` и bandit чисто.
...
#### R4.22 — t=16: 128-битная стойкость узла ПРЯМОЙ проверкой, в одной транзакции

Решение «отдельный t=16-верификатор (VFRI12) пропускается» **отменено замером.**

Обоснование пропуска было: «та же газовая стена, что у t=8, только в ~4 раза хуже — боевой V23
никогда не проверится». Оно опиралось на неоптимизированную перестановку — ровно та ошибка, которую
R4.8 уже исправил для t=8.

`QLSAVerifierVFRI12.sol` (протокол VFRI11 на бэкенде t=16: `Poseidon2MerkleVerifierT16` +
`Poseidon2ChannelT16`, узлы 8 слов / 248 бит → коллизия ~2^124):

| | t=8 (VFRI11) | t=16 (VFRI12) |
|---|---|---|
| одна перестановка (marginal) | 12 306 | **37 377** (3.04×, при вдвое более широком узле) |
| полный V23, LOG=10 | 3 342 623 | **9 036 930** |
| полный V23, LOG=8 | 2 633 375 | **6 511 542** |
| обе группы, ОДНА транзакция | 6 058 052 | **15 436 509** (лимит 16 777 216, запас 8%) |

**Область действия — важно не смешивать две границы.** 128 бит здесь — стойкость коллизии *узла
Меркла и транскрипта*. Стойкость FRI считается отдельно: `log_blowup(6) × n_queries + pow_bits(10)`.
Фикстуры используют `n_queries=1` (16 бит FRI), как и фикстура VFRI11 — чтобы числа были
сопоставимы. **Боевые 130 бит требуют 20 запросов, и при 20 запросах прямая проверка не влезает в
транзакцию ни при какой ширине хеша** (R4.17). Рекурсия остаётся единственным путём к FRI-границе;
t=16 поднимает хеш-границу. Одно не заменяет другое.

**Что стоило газа — снова не ширина.** Первая сборка: 3.57× от t=8 при отношении перестановок 3.04×.
Весь разрыв — число absorb'ов: канал унаследовал rate-1 от t=2/t=4/t=8, поэтому 8-словный корень
стоил восемь перестановок вместо одной. У губки t=16 rate = 8; rate-1 выбрасывает 7/8 пропускной
способности, ничего не давая по стойкости — границу задаёт capacity, а не rate. Rate-8 absorb:
LOG=10 −24%, LOG=8 −31%. Именно это завело пару под лимит.

**Найденный дефект паддинга.** Постоянный флаг оставлял `mix_u32s([1,2,3])` и `mix_u32s([1,2,3,0])`
в одном состоянии (оба дополняются до тех же восьми ячеек). Паддинг теперь добавляет `8-k`, кодируя
длину блока.

**Что ещё сделано.** Цепочка FRI параметризована по бэкенду (`P2Backend`/`P2Chan`, `T8Backend`/
`T16Backend`): VFRI11 и VFRI12 используют ОДНУ реализацию и не могут разъехаться с ABI-кодировщиком
(принцип R4.1). `verify()` у VFRI12 разделён на кадр проверок и рабочий кадр — с более широким
бэкендом Yul-аллокатор стека не справляется с формой VFRI11 («Variable size is 1 too deep»); ни
понижение бюджета инлайнинга, ни вынос только декодирования не помогают, потому что шесть
декодированных значений живут через всё тело. Это правка кодогенерации: проверки, их порядок и
транскрипт идентичны.

Мосты: `gen_vfri12_hints_from_cols_nfolds`, `gen_mldsa_v23_vfri12_hints[_log8]`,
`gen_mldsa_v23_vfri12_cross_bound_hints` (два прохода; инвариант «trace root не зависит от batch
root» **проверяется**, а не предполагается).

Тесты: 10 Rust (бэкенд) + 4 Rust (VFRI12) + 7 JS (перестановка) + 18 JS (бэкенд) + 11 JS (generic
E2E) + 11 JS (полный V23 cross-bound E2E). **538 Rust, 1085 Solidity, 0 падений.**

Лестница ширины узла завершена on-chain: t=2/t=4 (2^31) → t=8 (2^62) → **t=16 (2^124)**.

#### R4.23 — t=16 в РЕКУРСИИ не влезает; поправка к отношению перестановок

Естественный следующий шаг после R4.22 — перевести рекурсию на t=16 и получить обе боевые границы
разом (130 бит FRI от рекурсии + 2^124 на узлах от t=16). **Замер говорит: не влезает.**

Стоимость рекурсии on-chain определяется проверкой ВНЕШНЕГО доказательства — трейса самой
рекурсивной схемы, 87 колонок при `outer_log=14`, независимо от размера внутреннего утверждения.
На настоящем боевом внешнем трейсе (внутренние `n_queries=20`, `outer_folds = outer_log − 5 = 9`):

| внешняя проверка | газ |
|---|---|
| t=8 | 5 441 919 |
| t=16 | **16 044 328** — 96% лимита 16 777 216 |

Одна группа при t=16 съедает почти весь лимит, а `BatchRegistryV7` должен уместить ДВЕ плюс
воспроизведение канала и проверку последнего слоя внутреннего доказательства.

**Состояние границ на сегодня:**

| путь | FRI-стойкость | стойкость узла |
|---|---|---|
| рекурсия (t=8, стек v8) | **130 бит** | ~2^62 |
| VFRI12 напрямую | 16 бит (q=1; q=2 уже вне лимита) | **~2^124** |

Ни один путь не даёт обе. Гибрид не спасает: внутренние Меркл-узлы проверяются В СХЕМЕ (t=16 там
бесплатен по газу — `composition_t16` готов с R3.18), но ВНЕШНЕЕ доказательство проверяется
on-chain, и его ширина остаётся ограничивающей. Минимум по двум границам определяется внешней.

Нужно удешевить внешнюю проверку при t=16 примерно в 2.3×. Очевидные ручки этого не дают:
увеличение `outer_folds` уменьшает перестроение последнего слоя (32 листа), но растит работу по
Меркл-путям цепочки свёрток — суммарно порядка 1M.

Зонд: `contracts/test/OuterWidthProbe.test.js`, фикстура `outer_width_probe.json`
(`cargo test write_outer_width_probe -- --ignored`) — один и тот же внешний трейс, доказанный
дважды, так что разница между числами — только ширина хеша.

**Поправка к R4.22.** Записанное там отношение перестановок 1.79× неверно: оно получено из
`estimateGas` на одиночных внешних вызовах, где общие 21 000 газа базовой стоимости транзакции
приписываются обеим ширинам и занижают отношение. Маргинальная стоимость (`gas(n=2) − gas(n=1)`):
**t=8 = 12 306, t=16 = 37 377, отношение 3.04×**, то есть 151 газ на бит ёмкости узла против 99 —
t=16 в ~1.5 раза ДОРОЖЕ на бит, а не дешевле. Это и объясняет сквозные отношения 2.47–2.95×,
которые были видны с самого начала. Число 15 436 509 не затронуто: это `gasUsed` отправленной
транзакции. Вывод R4.22 («t=16 напрямую влезает в одну транзакцию») остаётся в силе; неверным было
обоснование «потому что на бит он дешевле» — верное обоснование в том, что t=16 достигает уровня,
недостижимого для t=8 вообще.
