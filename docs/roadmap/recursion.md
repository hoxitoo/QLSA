...
**Варианты решения** (требуют отдельного захода):

1. Поддержать переменную глубину пути в `merkle_path_t8_air` — раскладка строк становится
   зависимой от пути, селекторы `is_first_path`/`is_root` считаются по префиксным суммам, а не по
   `path_idx * depth * 22`.
2. Второй Merkle-компонент со СВОЕЙ глубиной. Препятствие: preprocessed-столбцы адресуются
   фиксированными строковыми ID (`mp_t8_*`), поэтому два экземпляра одного AIR в одном STARK
   столкнутся именами — потребуется параметризовать ID неймспейсом.

Вариант 2 выглядит чище (AIR не усложняется), но требует namespace-параметризации ID во всём
семействе `merkle_path_*`. Эксперимент откачен; закреплённая часть (R4.10) сохранена, поскольку она
самостоятельна и уже даёт выигрыш соунднеса.
