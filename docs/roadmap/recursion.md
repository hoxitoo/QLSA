...
**Вывод для порядка работ.** Стек v8 (Python/SDK-обвязка inner→outer, деплой, e2e) строить
**преждевременно**: на текущем конфиге он строго хуже v7. Сначала — замерить точку окупаемости по
`n_queries`. Если она подтвердится, рекурсия становится единственным путём к 130-битной стойкости
on-chain; если нет — рекурсия остаётся инструментом для 128-бит через t=16 inner hash, но не
средством экономии газа.
