"""
QLSA VFRI11 witness benchmark — prove_mldsa_sig_vfri11_stark per-transaction latency.

Measures end-to-end VFRI11 cross-bound ML-DSA-65 proving time: witness extraction
...
from stark.prover import prove_mldsa_sig_vfri11_stark
...
def _bench_single(n: int) -> dict:
    """Measure prove_mldsa_sig_vfri11_stark for each of n transactions, return stats."""
    ...
    for _ in range(REPEATS):
        ...
        for pk, msg, sig in txs_data:
            prove_mldsa_sig_vfri11_stark(pk, msg, sig, _BENCH_BATCH_ROOT, n_queries=1)
    ...
    return {
        ...
        "repeats": REPEATS,
        "prover": "vfri11_cross_bound",
    }


def run() -> None:
    if not _HAVE_EXT:
        print("qlsa_stark_stwo not installed — skipping VFRI11 witness benchmark.")
        print("Build with: cd stark_stwo && maturin develop --features python --release")
        return

    results = []
    print("VFRI11 cross-bound ML-DSA witness proving latency")
