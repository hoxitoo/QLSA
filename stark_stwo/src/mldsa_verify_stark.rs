...
// ── VerifyMldsaProofV18 — merged INTT+WPrime multi-component STARK (MVP-3+) ──
//
// Same as V17 but CombinedInttWPrimeBatch (2 sub-proofs: INTT-batch LOG=10 +
// WPrime LOG=8) is replaced by InttWPrimeProofV18 — one joint FRI polynomial
// covering both AIRs in a single multi-size commitment tree.
//
// Sub-proof breakdown:
//   AllNttProof:           1
//   AzCt1ProofV16:         1 (Az-full + Ct1-full)
//   InttWPrimeProofV18:    1 (INTT-batch + WPrime — multi-component, LOG=10/8)
//   NormUseHintProofV17:   1 (NormCheck + UseHintBatchV2)
// Total: 1+1+1+1 = 4 sub-proofs (vs 5 in V17)

/// Combined INTT-batch + WPrime proof (single multi-component STARK, mixed LOG sizes).
#[derive(Encode, Decode)]
pub struct InttWPrimeProofV18 {
    /// Bytes of the combined STARK proof.
    pub proof_combined:  Vec<u8>,
...
/// Prove INTT-batch AND WPrime in one multi-component STARK (InttWPrimeProofV18).
pub fn prove_intt_wprime_v18(
    az_hat:  &[[i64; N]],
    ct1_hat: &[[i64; N]],
    k:       usize,
) -> Result<InttWPrimeProofV18, String> {
    use crate::mldsa::params::K as K_PARAM;
    if k != K_PARAM { return Err(format!("prove_intt_wprime_v18 requires k=K={K_PARAM}, got {k}")); }
    ...
    let (proof_bytes, intt_cm, wp_cm, az_out, ct1_out, w_prime) =
        crate::prove_intt_wprime_combined(&az_arr, &ct1_arr)
            .map_err(|e| format!("prove_intt_wprime_combined failed: {e}"))?;

    Ok(InttWPrimeProofV18 {
        proof_combined:  proof_bytes,
...
/// Verify an InttWPrimeProofV18.
pub fn verify_intt_wprime_v18(
    proof:   &InttWPrimeProofV18,
    az_hat:  &[[i64; N]],
    ct1_hat: &[[i64; N]],
) -> Result<bool, String> {
    use crate::mldsa::params::K as K_PARAM;
    ...
    crate::verify_intt_wprime_combined(
        &proof.proof_combined,
        ...
        &ct1_out_arr,
        &w_prime_arr,
    ).map_err(|e| format!("verify_intt_wprime_combined failed: {e}"))
}

/// Combined proof — 4 sub-proofs (AllNtt + AzCt1 + InttWPrime + NormUseHintV17).
#[derive(Encode, Decode)]
pub struct VerifyMldsaProofV18 {
    pub ntt_proof:            AllNttProof,
    pub az_ct1_proof:         AzCt1ProofV16,
    pub intt_wprime_proof:    InttWPrimeProofV18,
    pub norm_use_hint_proof:  NormUseHintProofV17,
    pub c_tilde:              Vec<u8>,
}

/// Prove the full ML-DSA.Verify arithmetic witness (V18).
///
/// 4 sub-proofs (vs 5 in V17) — INTT-batch+WPrime merged into one multi-component STARK.
pub fn prove_verify_mldsa_v18(
    a_hat:   &[[i64; N]],
    z:       &[[i64; N]],
    c:       &[i64; N],
    t1:      &[[i64; N]],
    ...
    l:       usize,
    c_tilde: &[u8],
) -> Result<VerifyMldsaProofV18, String> {
    use crate::mldsa::params::{K as K_PARAM, L as L_PARAM};
    if k != K_PARAM { return Err(format!("V18 requires k=K={K_PARAM}, got {k}")); }
    if l != L_PARAM { return Err(format!("V18 requires l=L={L_PARAM}, got {l}")); }

    let ntt_proof         = prove_all_ntt(z, c, t1, l, k)?;
    let az_ct1_proof      = prove_az_ct1_v16(a_hat, &ntt_proof.z_hat, &ntt_proof.c_hat, &ntt_proof.t1_hat, k, l, c_tilde)?;
    let intt_wprime_proof = prove_intt_wprime_v18(&az_ct1_proof.az_hat, &az_ct1_proof.ct1_hat_out, k)?;
    let norm_use_hint_proof = prove_norm_use_hint_v17(z, &intt_wprime_proof.w_prime, hints)?;

    Ok(VerifyMldsaProofV18 {
        ntt_proof,
        az_ct1_proof,
        intt_wprime_proof,
        norm_use_hint_proof,
...
/// Verify all STARK sub-proofs in a `VerifyMldsaProofV18`.
pub fn verify_mldsa_witness_v18(proof: &VerifyMldsaProofV18) -> Result<bool, String> {
    use crate::mldsa::params::{K as K_PARAM, L as L_PARAM};
    use crate::mldsa::params::OMEGA;
...
// ── VerifyMldsaProofV19 — 3-component mixed-size NTT+Az+Ct1 STARK (MVP-3+) ────
//
// Merges AllNttProof + AzCt1ProofV16 (2 sub-proofs) into one 3-component
// mixed-size STARK covering all 12 NTTs (LOG=10), Az-full (LOG=8), and
// Ct1-full (LOG=8) in one FRI polynomial commitment.
//
// Sub-proof breakdown:
//   AllNttAzCt1ProofV19:     1 (NTT+Az+Ct1, LOG=10/8/8, 2467 cols)
//   InttWPrimeProofV18:      1 (INTT+WPrime, LOG=10/8)   ← unchanged from V18
//   NormUseHintProofV17:     1 (NormCheck+UseHintV2)      ← unchanged from V17
// Total: 3 sub-proofs (vs 4 in V18)

/// Combined NTT+Az+Ct1 proof (3-component mixed-size STARK).
#[derive(Encode, Decode)]
pub struct AllNttAzCt1ProofV19 {
    pub proof_combined:  Vec<u8>,
    ...
    /// t1_hat[i] = NTT(t1[i]), K entries.
    pub t1_hat:          Vec<[i64; N]>,
    /// NTT-domain Az outputs az_hat[i], K entries.
    pub az_hat:          Vec<[i64; N]>,
    /// NTT-domain Ct1 outputs ct1_hat[i], K entries.
    pub ct1_hat_out:     Vec<[i64; N]>,
}

/// Prove NTT(z,c,t1) + Az-full + Ct1-full in a single 3-component STARK.
pub fn prove_ntt_az_ct1_v19(
    a_hat:   &[[i64; N]],
    z:       &[[i64; N]],
    c:       &[i64; N],
    t1:      &[[i64; N]],
    ...
    l:       usize,
    c_tilde: &[u8],
) -> Result<AllNttAzCt1ProofV19, String> {
    use crate::mldsa::params::{K as K_PARAM, L as L_PARAM};
    if k != K_PARAM { return Err(format!("V19 requires k=K={K_PARAM}, got {k}")); }
    if l != L_PARAM { return Err(format!("V19 requires l=L={L_PARAM}, got {l}")); }
    ...
    let (proof_bytes, ntt_cm, az_cm, ct1_cm, z_hat, c_hat, t1_hat, az_out, ct1_out) =
        crate::prove_ntt_az_ct1_combined(
            &z_arr, c, &t1_arr, a_hat, c_tilde,
        ).map_err(|e| format!("prove_ntt_az_ct1_combined failed: {e}"))?;

    Ok(AllNttAzCt1ProofV19 {
        proof_combined: proof_bytes,
...
/// Verify an AllNttAzCt1ProofV19.
pub fn verify_ntt_az_ct1_v19(
    proof:   &AllNttAzCt1ProofV19,
    c_tilde: &[u8],
) -> Result<bool, String> {
    use crate::mldsa::params::K as K_PARAM;
    ...
    let az_hat_arr:  [[i64; N]; K_PARAM] = proof.az_hat.as_slice().try_into()
        .map_err(|_| "az_hat must have K entries".to_string())?;
    let ct1_hat_arr: [[i64; N]; K_PARAM] = proof.ct1_hat_out.as_slice().try_into()
        .map_err(|_| "ct1_hat_out must have K entries".to_string())?;

    // Cross-check NTT commitment: fingerprint of all 12 outputs must match.
    {
        ...
        for p in &proof.t1_hat { flat.extend_from_slice(p); }
        let expected_cm = crate::build_poly_commitment(&crate::output_fingerprint(&flat));
        if expected_cm != proof.ntt_commitment { return Ok(false); }
    }

    crate::verify_ntt_az_ct1_combined(
        &proof.proof_combined,
        ...
        &ct1_hat_arr,
        c_tilde,
    ).map_err(|e| format!("verify_ntt_az_ct1_combined failed: {e}"))
}

/// Combined proof — 3 sub-proofs (NttAzCt1 + InttWPrime + NormUseHint).
#[derive(Encode, Decode)]
pub struct VerifyMldsaProofV19 {
...
/// Prove the full ML-DSA.Verify arithmetic witness (V19).
///
/// 3 sub-proofs (vs 4 in V18) — NTT+Az+Ct1 merged into one mixed-size STARK.
pub fn prove_verify_mldsa_v19(
    a_hat:   &[[i64; N]],
    z:       &[[i64; N]],
    c:       &[i64; N],
    t1:      &[[i64; N]],
    ...
    l:       usize,
    c_tilde: &[u8],
) -> Result<VerifyMldsaProofV19, String> {
    use crate::mldsa::params::{K as K_PARAM, L as L_PARAM};
    if k != K_PARAM { return Err(format!("V19 requires k=K={K_PARAM}, got {k}")); }
    if l != L_PARAM { return Err(format!("V19 requires l=L={L_PARAM}, got {l}")); }

    // Step 1: prove NTT+Az+Ct1 together (pipeline NTT→Az, NTT→Ct1).
    let ntt_az_ct1_proof = prove_ntt_az_ct1_v19(a_hat, z, c, t1, k, l, c_tilde)?;

    // Step 2: prove INTT+WPrime using Az and Ct1 NTT-domain outputs.
    let intt_wprime_proof = prove_intt_wprime_v18(
        &ntt_az_ct1_proof.az_hat, &ntt_az_ct1_proof.ct1_hat_out, k,
    )?;

    // Step 3: prove NormCheck+UseHint using z and INTT w_prime outputs.
    let norm_use_hint_proof = prove_norm_use_hint_v17(z, &intt_wprime_proof.w_prime, hints)?;

    Ok(VerifyMldsaProofV19 {
        ntt_az_ct1_proof,
        intt_wprime_proof,
        norm_use_hint_proof,
        c_tilde: c_tilde.to_vec(),
...
/// Verify all STARK sub-proofs in a `VerifyMldsaProofV19`.
pub fn verify_mldsa_witness_v19(proof: &VerifyMldsaProofV19) -> Result<bool, String> {
    use crate::mldsa::params::OMEGA;
...
// ── VerifyMldsaProofV21 — 1 sub-proof: ALL 7 components in one STARK ─────────
//
// The theoretical minimum: NTT+Az+Ct1+INTT+WPrime+NormCheck+UseHint merged
// into a single FRI polynomial commitment (1 proof, 3216 main trace columns).
//
// Sub-proof breakdown:
//   VerifyMldsaProofV21:  1 (all 7 components, LOG=10/8/8/10/8/8/8)
// Total: 1 sub-proof (vs 2 in V20)

/// Single combined proof for all 7 ML-DSA.Verify components (V21).
#[derive(Encode, Decode)]
pub struct VerifyMldsaProofV21 {
    pub proof_combined:       Vec<u8>,
    // commitments (fingerprint-based)
    pub ntt_commitment:       String,
    pub az_commitment:        String,
    pub ct1_commitment:       String,
    pub intt_commitment:      String,
    ...
    pub norm_commitment:      String,
    pub use_hint_commitment:  String,
    // intermediate outputs needed for verification
    pub z_hat:                Vec<[i64; N]>,     // L NTT outputs for z
    pub c_hat:                [i64; N],
    pub t1_hat:               Vec<[i64; N]>,     // K NTT outputs for t1
    pub az_hat:               Vec<[i64; N]>,     // K NTT-domain Az outputs
    pub ct1_hat:              Vec<[i64; N]>,     // K NTT-domain Ct1 outputs
    pub az_out:               Vec<[i64; N]>,     // K INTT outputs for az
    pub ct1_out:              Vec<[i64; N]>,     // K INTT outputs for ct1
    pub w_prime:              Vec<[i64; N]>,     // K WPrime outputs
    pub max_norms:            Vec<i64>,          // L max norms from NormCheck
    pub output:               Vec<[i64; N]>,     // K UseHint w1_prime outputs
    pub hint_weight_total:    usize,
    pub c_tilde:              Vec<u8>,
}

/// Prove all 7 ML-DSA.Verify components in ONE STARK (V21).
pub fn prove_verify_mldsa_v21(
    a_hat:   &[[i64; N]],
    z:       &[[i64; N]],
    c:       &[i64; N],
    t1:      &[[i64; N]],
    ...
    l:       usize,
    c_tilde: &[u8],
) -> Result<VerifyMldsaProofV21, String> {
    use crate::mldsa::params::{K as K_PARAM, L as L_PARAM};
    if k != K_PARAM { return Err(format!("V21 requires k=K={K_PARAM}, got {k}")); }
    if l != L_PARAM { return Err(format!("V21 requires l=L={L_PARAM}, got {l}")); }
...
/// Verify a `VerifyMldsaProofV21`.
pub fn verify_mldsa_witness_v21(proof: &VerifyMldsaProofV21) -> Result<bool, String> {
    ...
    // NormCheck takes z in polynomial domain. Recover z = INTT(z_hat) and
    // t1 = INTT(t1_hat) and c = INTT(c_hat) for verify_full_mldsa_witness_combined.
...
// ── VerifyMldsaProofV23 — V22 + integrated RangeQBatch for az_hat soundness ──
//
// Extends V22 by adding RangeQBatch (288 columns, LOG=8) as an 8th component
// in the single FRI commitment.  The RangeQBatch proves az_hat[i][p] ∈ [0, Q)
// for all K output polynomials of the AzFull circuit, closing the primary
// soundness gap in the multiplication constraints.
//
// Sub-proof breakdown: same 7 components as V22 + RangeQBatch (1 FRI commitment)
// Total columns: 3216 main + 288 range-Q + 1 preproc = 3505 columns.

/// Single combined proof for all 8 ML-DSA.Verify components + Merkle root (V23).
#[derive(Encode, Decode)]
pub struct VerifyMldsaProofV23 {
    pub proof_combined:       Vec<u8>,
    pub ntt_commitment:       String,
    pub az_commitment:        String,
    pub ct1_commitment:       String,
...
/// Prove all 8 ML-DSA.Verify components in ONE STARK (V23).
///
/// V22 + RangeQBatch: proves az_hat ∈ [0, Q) as an 8th AIR component.
pub fn prove_verify_mldsa_v23(
    a_hat:       &[[i64; N]],
    z:           &[[i64; N]],
    c:           &[i64; N],
    t1:          &[[i64; N]],
    ...
    c_tilde:     &[u8],
    merkle_root: &[u8],
) -> Result<VerifyMldsaProofV23, String> {
    use crate::mldsa::params::{K as K_PARAM, L as L_PARAM};
    if k != K_PARAM { return Err(format!("V23 requires k=K={K_PARAM}, got {k}")); }
    if l != L_PARAM { return Err(format!("V23 requires l=L={L_PARAM}, got {l}")); }
    ...
    let (proof_bytes,
         ...
         w_prime, max_norms, w1_arr,
         hint_weight_total,
    ) = crate::prove_full_mldsa_witness_v23(
        &z_arr, c, &t1_arr, a_hat, &h_arr, c_tilde, merkle_root,
    ).map_err(|e| format!("prove_full_mldsa_witness_v23 (V23) failed: {e}"))?;

    Ok(VerifyMldsaProofV23 {
        proof_combined:      proof_bytes,
        ntt_commitment:      ntt_cm,
        az_commitment:       az_cm,
        ct1_commitment:      ct1_cm,
...
/// Verify a `VerifyMldsaProofV23`.
pub fn verify_mldsa_witness_v23(proof: &VerifyMldsaProofV23) -> Result<bool, String> {
    use crate::mldsa::params::{K as K_PARAM, L as L_PARAM, OMEGA};
    ...
    crate::verify_full_mldsa_witness_v23(
        &proof.proof_combined,
        &proof.ntt_commitment,
        &proof.az_commitment,
        &proof.ct1_commitment,
        ...
        &proof.c_tilde,
        &proof.merkle_root,
    ).map_err(|e| format!("verify_full_mldsa_witness_v23 (V23) failed: {e}"))
}
...
mod tests {
    ...
    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~2-4 min per test)"]
    fn test_prove_verify_mldsa_v18_roundtrip() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 28400); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 28500)).collect();
        let c:  [i64; N]       = random_poly(28600);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 28700)).collect();
        let h  = all_false_hints(k);
        let c_tilde: Vec<u8>   = (116u8..164).collect();

        let proof = prove_verify_mldsa_v18(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde)
            .expect("prove_verify_mldsa_v18 failed");

        assert_eq!(proof.norm_use_hint_proof.hint_weight_total, 0,
            "all-false hints must give hint_weight_total=0");
        assert!(verify_mldsa_witness_v18(&proof).expect("verify should not error"),
            "V18 proof must verify");
    }

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~2-4 min per test)"]
    fn test_prove_verify_mldsa_v18_matches_v17() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 28800); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 28900)).collect();
        let c:  [i64; N]       = random_poly(29000);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 29100)).collect();
        let h  = all_false_hints(k);
        let c_tilde: Vec<u8>   = (117u8..165).collect();

        let proof_v17 = prove_verify_mldsa_v17(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde)
            .expect("prove_verify_mldsa_v17 failed");
        let proof_v18 = prove_verify_mldsa_v18(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde)
            .expect("prove_verify_mldsa_v18 failed");

        assert!(verify_mldsa_witness_v17(&proof_v17).expect("V17 verify error"), "V17 must verify");
        assert!(verify_mldsa_witness_v18(&proof_v18).expect("V18 verify error"), "V18 must verify");

        // Both must produce identical INTT and WPrime outputs.
        assert_eq!(proof_v17.intt_wprime_proof.az_out, proof_v18.intt_wprime_proof.az_out,
            "V17 and V18 az_out must match");
        assert_eq!(proof_v17.intt_wprime_proof.ct1_out, proof_v18.intt_wprime_proof.ct1_out,
            "V17 and V18 ct1_out must match");
        assert_eq!(proof_v17.intt_wprime_proof.w_prime, proof_v18.intt_wprime_proof.w_prime,
            "V17 and V18 w_prime must match");
    }

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~2-4 min per test)"]
    fn test_prove_verify_mldsa_v18_c_tilde_binding() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 29200); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 29300)).collect();
        let c:  [i64; N]       = random_poly(29400);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 29500)).collect();
        let h  = all_false_hints(k);

        let c_tilde_a: Vec<u8> = (118u8..166).collect();
        let c_tilde_b: Vec<u8> = (119u8..167).collect();

        let mut proof = prove_verify_mldsa_v18(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde_a)
            .expect("prove_verify_mldsa_v18 failed");

        assert!(verify_mldsa_witness_v18(&proof).expect("verify should not error"),
            "V18 must verify with original c_tilde");

        proof.c_tilde = c_tilde_b;
        assert!(!verify_mldsa_witness_v18(&proof).expect("verify should not error after tamper"),
            "tampered c_tilde must cause V18 verification failure");
    }
    ...
    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~2-4 min per test)"]
    fn test_prove_verify_mldsa_v19_roundtrip() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 29600); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 29700)).collect();
        let c:  [i64; N]       = random_poly(29800);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 29900)).collect();
        let h  = all_false_hints(k);
        let c_tilde: Vec<u8>   = (120u8..168).collect();

        let proof = prove_verify_mldsa_v19(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde)
            .expect("prove_verify_mldsa_v19 failed");

        assert_eq!(proof.norm_use_hint_proof.hint_weight_total, 0,
            "all-false hints must give hint_weight_total=0");
        assert!(verify_mldsa_witness_v19(&proof).expect("verify should not error"),
            "V19 proof must verify");
    }

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~2-4 min per test)"]
    fn test_prove_verify_mldsa_v19_matches_v18() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 30000); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 30100)).collect();
        let c:  [i64; N]       = random_poly(30200);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 30300)).collect();
        let h  = all_false_hints(k);
        let c_tilde: Vec<u8>   = (121u8..169).collect();
        ...
        // Both must produce identical NTT outputs.
        assert_eq!(proof_v18.ntt_proof.z_hat, proof_v19.ntt_az_ct1_proof.z_hat,
            "V18 and V19 z_hat must match");
        // Both must produce identical INTT+WPrime outputs.
        assert_eq!(proof_v18.intt_wprime_proof.w_prime, proof_v19.intt_wprime_proof.w_prime,
            "V18 and V19 w_prime must match");
        // Both must produce identical NormCheck+UseHint outputs.
        assert_eq!(proof_v18.norm_use_hint_proof.max_norms, proof_v19.norm_use_hint_proof.max_norms,
            "max_norms must match between V18 and V19");
        assert_eq!(
            proof_v18.norm_use_hint_proof.output,
            proof_v19.norm_use_hint_proof.output,
            "UseHint outputs must match between V18 and V19"
        );
    }

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~2-4 min per test)"]
    fn test_prove_verify_mldsa_v19_c_tilde_binding() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 30400); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 30500)).collect();
        let c:  [i64; N]       = random_poly(30600);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 30700)).collect();
        let h  = all_false_hints(k);

        let c_tilde_a: Vec<u8> = (122u8..170).collect();
        let c_tilde_b: Vec<u8> = (123u8..171).collect();

        let mut proof = prove_verify_mldsa_v19(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde_a)
            .expect("prove_verify_mldsa_v19 failed");

        assert!(verify_mldsa_witness_v19(&proof).expect("verify should not error"),
            "V19 must verify with original c_tilde");
    ...
    // ── V21 tests (1 sub-proof: all 7 components in one STARK) ───────────────

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~3-5 min per test)"]
    fn test_prove_verify_mldsa_v21_roundtrip() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 32000); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 32100)).collect();
        let c:  [i64; N]       = random_poly(32200);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 32300)).collect();
        let h  = all_false_hints(k);
        let c_tilde: Vec<u8>   = (140u8..188).collect();

        let proof = prove_verify_mldsa_v21(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde)
            .expect("prove_verify_mldsa_v21 failed");

        assert!(verify_mldsa_witness_v21(&proof).expect("V21 verify error"),
            "V21 must verify");
        assert_eq!(proof.hint_weight_total, 0, "all-false hints → weight 0");
        assert_eq!(proof.az_hat.len(), k, "az_hat must have K entries");
        assert_eq!(proof.w_prime.len(), k, "w_prime must have K entries");
        assert_eq!(proof.output.len(), k, "output must have K entries");
    }

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~3-5 min per test)"]
    fn test_prove_verify_mldsa_v21_matches_v20() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 32400); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 32500)).collect();
        let c:  [i64; N]       = random_poly(32600);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 32700)).collect();
        let h  = all_false_hints(k);
        let c_tilde: Vec<u8>   = (141u8..189).collect();
        ...
        assert!(verify_mldsa_witness_v20(&proof_v20).expect("V20 verify error"), "V20 must verify");
        assert!(verify_mldsa_witness_v21(&proof_v21).expect("V21 verify error"), "V21 must verify");

        // NTT outputs must match between V20 and V21.
        assert_eq!(proof_v20.ntt_az_ct1_proof.z_hat, proof_v21.z_hat,
            "z_hat must match between V20 and V21");
        // Az/Ct1 NTT-domain outputs must match.
        assert_eq!(proof_v20.ntt_az_ct1_proof.az_hat, proof_v21.az_hat,
            "az_hat must match between V20 and V21");
        // INTT+WPrime outputs must match.
        assert_eq!(proof_v20.intt_wp_norm_uh_proof.w_prime, proof_v21.w_prime,
            "w_prime must match between V20 and V21");
        // NormCheck outputs must match.
        assert_eq!(proof_v20.intt_wp_norm_uh_proof.max_norms, proof_v21.max_norms,
            "max_norms must match between V20 and V21");
        // UseHint outputs must match.
        assert_eq!(proof_v20.intt_wp_norm_uh_proof.output, proof_v21.output,
            "UseHint outputs must match between V20 and V21");
    }

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~3-5 min per test)"]
    fn test_prove_verify_mldsa_v21_c_tilde_binding() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 32800); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 32900)).collect();
        let c:  [i64; N]       = random_poly(33000);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 33100)).collect();
        let h  = all_false_hints(k);

        let c_tilde_a: Vec<u8> = (142u8..190).collect();
        let c_tilde_b: Vec<u8> = (143u8..191).collect();

        let mut proof = prove_verify_mldsa_v21(&a_hat, &z, &c, &t1, &h, k, l, &c_tilde_a)
            .expect("prove_verify_mldsa_v21 failed");

        assert!(verify_mldsa_witness_v21(&proof).expect("verify should not error"),
            "V21 must verify with original c_tilde");
    ...
    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~3-5 min per test)"]
    fn test_prove_verify_mldsa_v23_roundtrip() {
        let k = crate::mldsa::params::K;
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 35200); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 35300)).collect();
        let c:  [i64; N]       = random_poly(35400);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 35500)).collect();
        let h  = all_false_hints(k);
        let c_tilde:     Vec<u8> = (155u8..203).collect();
        let merkle_root: Vec<u8> = (210u8..242).collect();  // 32-byte root

        let proof = prove_verify_mldsa_v23(&a_hat, &z, &c, &t1, &h, k, l,
            &c_tilde, &merkle_root)
            .expect("prove_verify_mldsa_v23 failed");

        assert!(verify_mldsa_witness_v23(&proof).expect("V23 verify error"), "V23 must verify");
        assert_eq!(proof.hint_weight_total, 0, "all-false hints → weight 0");
        assert_eq!(proof.merkle_root, merkle_root, "merkle_root stored in proof");
        // az_hat must have K=6 entries.
        assert_eq!(proof.az_hat.len(), k, "az_hat must have K entries");
    }

    #[test]
    #[ignore = "slow: runs full STARK proof pipeline (~3-5 min per test)"]
    fn test_prove_verify_mldsa_v23_range_q_binding() {
        // Corrupt az_hat by injecting a value >= Q into the inputs and verify
        // that the prover rejects it (rq_valid=false → Err).
        // We achieve this by constructing a_hat entry with an out-of-range value
        // directly; since az_build_trace normalises into [0,Q), we instead call
        // prove_full_mldsa_witness_v23 with corrupted a_hat that would yield
        // az_hat outside [0,Q) — the simplest way is to just call with valid
        // inputs (which always land in [0,Q)), then separately verify that
        // rq_build_trace returns valid=false for an out-of-range poly.
        use crate::mldsa_range_q_batch_air;

        let k = crate::mldsa::params::K;
        // Normal valid roundtrip check.
        let l = crate::mldsa::params::L;
        let a_hat: Vec<[i64; N]> = (0..k*l)
            .map(|s| { let mut h = random_poly(s as u64 + 36000); ntt(&mut h); h })
            .collect();
        let z:  Vec<[i64; N]>  = (0..l).map(|s| random_poly(s as u64 + 36100)).collect();
        let c:  [i64; N]       = random_poly(36200);
        let t1: Vec<[i64; N]>  = (0..k).map(|s| random_poly(s as u64 + 36300)).collect();
        let h  = all_false_hints(k);
        let c_tilde:     Vec<u8> = (156u8..204).collect();
        let merkle_root: Vec<u8> = (211u8..243).collect();

        let proof = prove_verify_mldsa_v23(&a_hat, &z, &c, &t1, &h, k, l,
            &c_tilde, &merkle_root)
            .expect("prove_verify_mldsa_v23 failed");
        assert!(verify_mldsa_witness_v23(&proof).expect("V23 verify error"),
            "V23 valid proof must verify");

        // Verify that rq_build_trace correctly rejects an out-of-range polynomial.
        let mut bad_polys: [[i64; N]; crate::mldsa::params::K] =
            std::array::from_fn(|j| random_poly(j as u64 * 7 + 100));
        // Ensure all values are in [0, Q).
        for poly in &mut bad_polys {
            for v in poly.iter_mut() {
                *v = v.rem_euclid(crate::mldsa::Q);
            }
        }
        // Corrupt one value to be >= Q.
        bad_polys[0][0] = crate::mldsa::Q;
        let (_, rq_valid) = mldsa_range_q_batch_air::build_trace(&bad_polys);
        assert!(!rq_valid, "rq_build_trace must return valid=false when v >= Q");
    }
}
