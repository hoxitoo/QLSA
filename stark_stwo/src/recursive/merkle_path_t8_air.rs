//! Poseidon2 **t=8** Merkle authentication-path AIR — wide recursion inner hash.
//!
//! The wide analogue of [`super::merkle_path_air`] (t=2, 31-bit nodes): proves a
//! Merkle authentication path over **4-word (124-bit) M31 nodes** using the
//! Poseidon2 t=8 2-to-1 compression [`crate::poseidon2_t8::compress_t8`] (node
//! collision ~2^62 vs t=2's ~2^15.5).  This is the hash a **VFRI11** inner proof's
...
//! cur_{i+1}         = compress_t8(left_i, right_i)
//! root              = cur_D
//! ```
//!
//! # Trace layout (45 main columns + 22 preprocessed)
//!
//! Each compression takes `N_ROUNDS = 22` rows (one t=8 permutation round each);
...
//! ```text
//! Main (per row):
//!   in[0..8] sq[0..8] sbox[0..8] out[0..8]   — the t=8 round columns (32)
//!   cur[0..4]  — chained current node (leaf at path start, else prev comp output)
//!   sib[0..4]  — sibling node       (meaningful on each compression's row 0)
//!   bit        — index bit b_i       (meaningful on each compression's row 0)
//!   leaf[0..4] — the path leaf        (meaningful on the path's very first row)
//!
//! Preprocessed (verifier-pinned canonical source, C1/C2):
//!   rc[0..8]        — round constants (period 22; cell 0 only on internal rounds)
...
//!   leaf_pin[0..4]  — verifier-fixed leaf                  (C1 leaf binding)
//!   is_root         — 1 on the last real compression's last round row
//!   root_pin[0..4]  — verifier-fixed root                  (C1 root binding)
//! ```
//!
//! # Soundness
//!
...
//!   `Poseidon2MerkleVerifierT8.verify(root, leaf, index, depth, siblings)`.
...
use crate::poseidon2_t8::{compress_t8, mat_external as mat_external_ref, mat_internal as mat_internal_ref, T};
...
pub const N_MAIN_COLS: usize = 45;
...
pub type MerklePathT8Component = FrameworkComponent<MerklePathT8Eval>;

// ── Column offsets ───────────────────────────────────────────────────────────
const C_IN: usize = 0; // 0..8
const C_SQ: usize = 8; // 8..16
const C_SBOX: usize = 16; // 16..24
const C_OUT: usize = 24; // 24..32
const C_CUR: usize = 32; // 32..36
const C_SIB: usize = 36; // 36..40
const C_BIT: usize = 40; // 40
const C_LEAF: usize = 41; // 41..45

// ── Preprocessed column IDs ───────────────────────────────────────────────────

pub fn pc_rc(i: usize) -> PreProcessedColumnId {
    PreProcessedColumnId { id: format!("mpt8_rc{i}") }
}
pub fn pc_is_ext() -> PreProcessedColumnId { PreProcessedColumnId { id: "mpt8_is_ext".into() } }
pub fn pc_is_int() -> PreProcessedColumnId { PreProcessedColumnId { id: "mpt8_is_int".into() } }
pub fn pc_is_first_comp() -> PreProcessedColumnId { PreProcessedColumnId { id: "mpt8_is_first_comp".into() } }
pub fn pc_is_first_path() -> PreProcessedColumnId { PreProcessedColumnId { id: "mpt8_is_first_path".into() } }
pub fn pc_idx_bit() -> PreProcessedColumnId { PreProcessedColumnId { id: "mpt8_idx_bit".into() } }
pub fn pc_leaf(k: usize) -> PreProcessedColumnId { PreProcessedColumnId { id: format!("mpt8_leaf{k}") } }
pub fn pc_is_root() -> PreProcessedColumnId { PreProcessedColumnId { id: "mpt8_is_root".into() } }
pub fn pc_root(k: usize) -> PreProcessedColumnId { PreProcessedColumnId { id: format!("mpt8_root{k}") } }
...
/// `sibs` using the index `bits` (LSB first), via `compress_t8`.
pub fn merkle_path_root_t8(leaf: [u64; 4], sibs: &[[u64; 4]], bits: &[bool]) -> [u64; 4] {
    assert_eq!(sibs.len(), bits.len(), "sibs/bits length mismatch");
    let mut cur = norm4(leaf);
    for i in 0..sibs.len() {
        let s = norm4(sibs[i]);
...
pub struct MerklePathT8Eval {
    pub log_n_rows: u32,
}

impl FrameworkEval for MerklePathT8Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let leaf_pin: Vec<E::F> = (0..4).map(|k| eval.get_preprocessed_column(pc_leaf(k))).collect();
        let is_root = eval.get_preprocessed_column(pc_is_root());
        let root_pin: Vec<E::F> = (0..4).map(|k| eval.get_preprocessed_column(pc_root(k))).collect();
        ...
        let cur: Vec<E::F> =
            (0..4).map(|_| eval.next_interaction_mask(ORIGINAL_TRACE_IDX, [0_isize])[0].clone()).collect();
        let sib: Vec<E::F> =
            (0..4).map(|_| eval.next_interaction_mask(ORIGINAL_TRACE_IDX, [0_isize])[0].clone()).collect();
...
pub fn prove_merkle_path_t8(
    leaf: [u64; 4],
    sibs: &[[u64; 4]],
    bits: &[bool],
) -> Result<(Vec<u8>, u32, [u64; 4]), String> {
...
/// Verify a proof from [`prove_merkle_path_t8`] against `(depth, leaf, index, root)`.
pub fn verify_merkle_path_t8(
    proof_bytes: &[u8],
    log_size: u32,
    depth: usize,
    leaf: [u64; 4],
...
mod tests {
    ...
    fn rand_path(seed: &mut u64, depth: usize) -> ([u64; 4], Vec<[u64; 4]>, Vec<bool>) {
        let leaf = rand_node(seed);
        let sibs: Vec<[u64; 4]> = (0..depth).map(|_| rand_node(seed)).collect();
        let bits: Vec<bool> = (0..depth).map(|_| rand_m31(seed) & 1 == 1).collect();
        (leaf, sibs, bits)
    }

    // Reference root vs direct compress_t8 chaining, both bit orderings.
    #[test]
    fn test_path_root_matches_manual_compress() {
        let leaf = [1u64, 2, 3, 4];
        let s0 = [10u64, 11, 12, 13];
        let s1 = [20u64, 21, 22, 23];
        // bits [false, true]: h1 = compress_t8(leaf, s0); root = compress_t8(s1, h1)
        let h1 = compress_t8(leaf, s0);
        let expected = compress_t8(s1, h1);
        assert_eq!(merkle_path_root_t8(leaf, &[s0, s1], &[false, true]), expected);
    }
    ...
    fn test_build_trace_root_matches_reference() {
        ...
        for depth in [1usize, 2, 3, 5] {
            ...
            assert_eq!(root, merkle_path_root_t8(leaf, &sibs, &bits), "trace root must match reference");
        }
    }
    ...
    fn test_roundtrip_depth1() {
        ...
        let (proof, log, root) = prove_merkle_path_t8(leaf, &sibs, &bits).expect("prove");
        let idx = bits_to_index(&bits);
        assert!(verify_merkle_path_t8(&proof, log, sibs.len(), leaf, idx, root).expect("verify"));
    }
    ...
    fn test_roundtrip_depth3() {
        ...
        let (proof, log, root) = prove_merkle_path_t8(leaf, &sibs, &bits).expect("prove");
        let idx = bits_to_index(&bits);
        assert!(verify_merkle_path_t8(&proof, log, sibs.len(), leaf, idx, root).expect("verify"), "valid path must verify");
    }
    ...
    fn test_wrong_root_rejected() {
        ...
        let mut bad = root;
        bad[0] ^= 1;
        assert!(!verify_merkle_path_t8(&proof, log, sibs.len(), leaf, idx, bad).unwrap_or(false), "wrong root must not verify");
    }
    ...
    fn test_wrong_index_rejected() {
        ...
        let (proof, log, root) = prove_merkle_path_t8(leaf, &sibs, &bits).expect("prove");
        let idx = bits_to_index(&bits);
        assert!(!verify_merkle_path_t8(&proof, log, sibs.len(), leaf, idx ^ 1, root).unwrap_or(false), "wrong index must not verify");
    }
    ...
    fn test_wrong_leaf_rejected() {
        ...
        let mut bad = leaf;
        bad[0] ^= 1;
        assert!(!verify_merkle_path_t8(&proof, log, sibs.len(), bad, idx, root).unwrap_or(false), "wrong leaf must not verify");
    }
    ...
    fn test_tampered_proof_rejected() {
        ...
        assert!(!verify_merkle_path_t8(&bad, log, sibs.len(), leaf, idx, root).unwrap_or(false), "tampered proof must not verify");
    }
    ...
    fn test_forged_preproc_rejected() {
        ...
        if let Ok(proof) = prove::<CpuBackend, Blake2sM31MerkleChannel>(&[&component], channel, scheme) {
            ...
            assert!(!verify_merkle_path_t8(&bytes, log, sibs.len(), leaf, idx, root).unwrap_or(false), "forged preproc must not verify (C2)");
        }
    }
}
