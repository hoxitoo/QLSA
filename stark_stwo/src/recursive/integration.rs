...
mod tests {
    ...
    use crate::recursive::recursive_verifier::{
        prove_recursive_query, query_alphas, recursive_query_final, verify_recursive_query, FoldRound, StepOp,
    };
    ...
    fn test_end_to_end_one_query() {
        ...
        assert!(
            verify_recursive_query(&rv_bytes, rv_log, rounds.len(), &query_alphas(&step, &rounds), px, final_fold).unwrap(),
    ...
    fn test_tampered_final_fold_breaks_chain() {
        ...
        assert!(!verify_recursive_query(&rv_bytes, rv_log, rounds.len(), &query_alphas(&step, &rounds), px, wrong).unwrap_or(false));
