...
pub fn build_trace(
    ...
    let used = 1 + rounds.len();
    debug_assert!(used <= n, "rows exceed trace capacity");
    ...
    cols[0][0] = BaseField::from_u32_unchecked(px);
    set(&mut cols, 1, 0, z_x);
    set(&mut cols, 5, 0, fri_alpha);
    cols[9][0] = BaseField::from_u32_unchecked(y_inv);
    set(&mut cols, 10, 0, f_plus); // a = fPlus
    set(&mut cols, 14, 0, f_minus); // b = fMinus
    set(&mut cols, 18, 0, combo_pos);
    set(&mut cols, 22, 0, combo_neg);
    set(&mut cols, 26, 0, comp_pos);
    set(&mut cols, 30, 0, comp_neg);
    set(&mut cols, 34, 0, pack(p0));
    set(&mut cols, 38, 0, outs[0]);
    is_step_col[0] = BaseField::from_u32_unchecked(1);
    ...
    for (i, &(sibling, alpha, x_inv)) in rounds.iter().enumerate() {
        ...
        // a = input, b = sibling, alpha = round alpha, inv = xInv
        set(&mut cols, 5, r, alpha);
        cols[9][r] = BaseField::from_u32_unchecked(x_inv);
        set(&mut cols, 10, r, input);
        set(&mut cols, 14, r, sibling);
        set(&mut cols, 34, r, pack(p_r));
        set(&mut cols, 38, r, outs[r]);
