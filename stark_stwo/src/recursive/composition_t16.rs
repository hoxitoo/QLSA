//! Recursive composition on the **128-bit (t=16) inner hash** — per-query verification
//! + t=8 Merkle membership in ONE STARK (R3.18).
//!
...
//! - the verifier computes `leaf8 = qm31_leaf_hash_t16(finalFold)` off-circuit (a
//!   deterministic public function of the *pinned* finalFold) and pins it in
//!   `merkle_path_t8`'s 8-word `leaf` columns (C1 leaf binding);
//! - `merkle_path_t8` pins the claimed `root` in-circuit (C1 root binding) and
//!   authenticates `leaf8 @ index + siblings → root` over 8-word nodes;
...
//! step that lifts the recursion's inner-hash node collision to ~2^124 ≈ 128-bit; This completes the
//! inner-hash ladder in-circuit (t=2 → t=8 → t=16).
...
use crate::recursive::integration::qm31_leaf_hash_t16;
use crate::recursive::merkle_path_t16_air as merkle;
...
/// over 8-word nodes.
pub fn prove_query_membership_t16(
    step: &QueryStep,
    rounds: &[FoldRound],
    sibs: &[[u64; 8]],
    bits: &[bool],
) -> Result<QueryMembershipT16Result, String> {
    ...
    let leaf = qm31_leaf_hash_t16(final_fold);
    ...
    debug_assert_eq!(root, merkle::merkle_path_root_t16(leaf, sibs, bits));
    ...
    let merkle_comp = merkle::MerklePathT16Component::new(
        &mut alloc,
        merkle::MerklePathT16Eval { log_n_rows: log_size },
        SecureField::from(0u32),
    );
...
/// by the verifier as `qm31_leaf_hash_t16(final_fold)` and pinned — so the t=8
/// Merkle path authenticates exactly the wide hash of the fold-chain output.
#[allow(clippy::too_many_arguments)]
pub fn verify_query_membership_t16(
    proof_bytes: &[u8],
    log_size: u32,
    num_folds: usize,
    depth: usize,
...
fn mix_public_queries(
    ...
    for p in 0..leaves.len() {
        ...
        mwords.extend(roots[p].iter().map(|&v| w(v)));
    }
    channel.mix_u32s(&mwords);
}

/// Result of an N-query t=8 composition proof.
pub struct QueriesMembershipT16Result {
    pub proof: Vec<u8>,
    pub log_size: u32,
    pub num_folds: usize,
    pub depth: usize,
...
pub fn prove_queries_membership_t16(
    queries: &[(QueryStep, Vec<FoldRound>)],
    paths: &[(Vec<[u64; 8]>, Vec<bool>)],
) -> Result<QueriesMembershipT16Result, String> {
    if queries.is_empty() {
    ...
    let leaves: Vec<[u64; 8]> = finals.iter().map(|&f| qm31_leaf_hash_t16(f)).collect();
    let sibs: Vec<Vec<[u64; 8]>> = paths.iter().map(|(s, _)| s.clone()).collect();
...
/// I/O. Leaves are recomputed as `qm31_leaf_hash_t16(final)` and pinned.
#[allow(clippy::too_many_arguments)]
pub fn verify_queries_membership_t16(
    proof_bytes: &[u8],
    log_size: u32,
    num_folds: usize,
    depth: usize,
...
mod tests {
    ...
    #[test]
    fn test_composition_t16_roundtrip() {
        let mut s = 0xC0117_16_u64;
        let step = sample_step(&mut s);
        let rounds = sample_rounds(&mut s, 6);
        let depth = 2;
        let sibs: Vec<[u64; 8]> = (0..depth).map(|_| rand_node(&mut s)).collect();
        let bits: Vec<bool> = (0..depth).map(|_| rand_m31(&mut s) & 1 == 1).collect();

        let r = prove_query_membership_t16(&step, &rounds, &sibs, &bits).unwrap();
        assert_eq!(r.final_fold, rv::recursive_query_final(&step, &rounds));
        assert_eq!(r.leaf, qm31_leaf_hash_t16(r.final_fold));
        assert_eq!(r.root, merkle::merkle_path_root_t16(r.leaf, &sibs, &bits));
        assert!(
    ...
    #[test]
    fn test_composition_t16_wrong_final_rejected() {
        let mut s = 0xBADC0_16_u64;
        let step = sample_step(&mut s);
        ...
        let r = prove_query_membership_t16(&step, &rounds, &sibs, &bits).unwrap();
        assert!(verify_query_membership_t16(&r.proof, r.log_size, r.num_folds, r.depth, &r.challenges, step.2, r.final_fold, r.index, r.root).unwrap());
        assert!(
            !verify_query_membership_t16(&r.proof, r.log_size, r.num_folds, r.depth, &r.challenges, step.2, r.final_fold ^ 1, r.index, r.root)
                .unwrap_or(false),
    ...
    fn test_queries_membership_t16_roundtrip() {
        ...
        let r = prove_queries_membership_t16(&queries, &paths).unwrap();
        ...
        for (i, (st, rd)) in queries.iter().enumerate() {
            ...
            assert_eq!(r.leaves[i], qm31_leaf_hash_t16(r.finals[i]));
            assert_eq!(r.roots[i], merkle::merkle_path_root_t16(r.leaves[i], &paths[i].0, &paths[i].1));
        }
        assert!(
            verify_queries_membership_t16(&r.proof, r.log_size, r.num_folds, r.depth, &r.challenges, &pxs, &r.finals, &r.indices, &r.roots)
                .unwrap(),
            "an honest N-query t=8 composition must verify",
        );
        // A wrong final for one query must fail (changes its pinned fin + leaf8).
        let mut bad = r.finals.clone();
        bad[1] ^= 1;
        assert!(
            !verify_queries_membership_t16(&r.proof, r.log_size, r.num_folds, r.depth, &r.challenges, &pxs, &bad, &r.indices, &r.roots)
                .unwrap_or(false),
            "a wrong final fold must not verify",
        );
        ...
        bad_roots[2][0] ^= 1;
        assert!(
            !verify_queries_membership_t16(&r.proof, r.log_size, r.num_folds, r.depth, &r.challenges, &pxs, &r.finals, &r.indices, &bad_roots)
                .unwrap_or(false),
    ...
    #[test]
    fn test_queries_membership_t16_validation_errors() {
        let mut s = 0xE44_u64;
        let step = sample_step(&mut s);
        let rounds = sample_rounds(&mut s, 2);
        // depth = 0 path
        let bad_paths: Vec<(Vec<[u64; 8]>, Vec<bool>)> = vec![(vec![], vec![])];
        assert!(prove_queries_membership_t16(&[(step, rounds.clone())], &bad_paths).is_err());
        // queries/paths length mismatch
        let ok_path: Vec<(Vec<[u64; 8]>, Vec<bool>)> =
            vec![(vec![rand_node(&mut s)], vec![true]), (vec![rand_node(&mut s)], vec![false])];
        assert!(prove_queries_membership_t16(&[(step, rounds)], &ok_path).is_err());
        // verify: log_size out of range
        assert!(
            verify_queries_membership_t16(&[], 40, 2, 1, &[], &[], &[1u128], &[0], &[[0u64; 8]]).is_err()
        );
    }

    // A wrong claimed root (not the one the t=8 path hashes to) must not verify.
    #[test]
    fn test_composition_t16_wrong_root_rejected() {
        let mut s = 0xBAD8007_16_u64;
        let step = sample_step(&mut s);
        ...
        bad[0] ^= 1;
        assert!(
            !verify_query_membership_t16(&r.proof, r.log_size, r.num_folds, r.depth, &r.challenges, step.2, r.final_fold, r.index, bad)
                .unwrap_or(false),
